...
DEFINE_LOG_CATEGORY(LogPhotonAndOVR);
...
bool UKismetTextLibrary::ParseLocationAndRotation(const FString& LocationAndRotationString, FVector& Location, FRotator& Rotation)
{
...
void UKismetTextLibrary::GetIsLobby(const FString& DeepLink, bool& bIsLobby)
{
...
void UKismetTextLibrary::GetLaunchMap(const FString& DeepLink, const FString& OptionalDefaultMapName, bool& HasMapName, FString& MapName)
{
	TSharedPtr<FJsonObject> DeepLinkJson = MakeShareable(new FJsonObject());
	TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(DeepLink);
...
void UKismetTextLibrary::GetPublicRoomName(const FString& DeepLink, bool& HasPublicRoomName, FString& PublicRoomName)
{
...
FString UKismetTextLibrary::GetLocalCharacterLocationAndRotation()
{
...
void UKismetTextLibrary::SystemLog(const FString& Message)
{
	UE_LOG(LogPhotonAndOVR, Display, TEXT("%s"), *Message);
}
