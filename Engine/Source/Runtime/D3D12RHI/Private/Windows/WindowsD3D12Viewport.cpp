...
void FD3D12Viewport::Init()
{
	...
	{
		if (ProviderModule->SupportsRHI(GDynamicRHI->GetName()))
		{
	...
	if (bNeedSwapChain)
	{
		if (Adapter->GetOwningRHI()->IsQuadBufferStereoEnabled())
		{
			...
			if (bIsStereoEnabled)
			{
				...
				HRESULT hr = DXGISwapchainProvider ?
					DXGISwapchainProvider->CreateSwapChainForHwnd(Factory2, pCommandQueue, WindowHandle, &SwapChainDesc1, nullptr, nullptr, SwapChain1.GetInitReference()) :
					Factory2->CreateSwapChainForHwnd(pCommandQueue, WindowHandle, &SwapChainDesc1, nullptr, nullptr, SwapChain1.GetInitReference());
		...
		if (SwapChain1 == nullptr)
		{
			// Create the swapchain.
			{
				...
				HRESULT hr = DXGISwapchainProvider ?
					DXGISwapchainProvider->CreateSwapChain(Adapter->GetDXGIFactory2(), pCommandQueue, &SwapChainDesc, SwapChain.GetInitReference()) :
					Adapter->GetDXGIFactory2()->CreateSwapChain(pCommandQueue, &SwapChainDesc, SwapChain.GetInitReference());
				if (FAILED(hr))
				{
