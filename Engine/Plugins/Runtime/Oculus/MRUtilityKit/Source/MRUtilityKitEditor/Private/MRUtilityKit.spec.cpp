...
bool TestEqual(const TCHAR* What, const FVector2D Actual, const FVector2D Expected, float Tolerance = SMALL_NUMBER);
...
void FMRUKSpec::Define()
{
	...
	Describe(TEXT("Serialization"), [this] {
		...
		LatentIt(TEXT("Deserializes correctly"), [this](const FDoneDelegate& Done) {
			...
			if (TestNotNull(TEXT("Floor anchor"), Room->FloorAnchor))
			{
			...
			if (TestNotNull(TEXT("Ceiling anchor"), Room->CeilingAnchor))
			{
			...
			{
				if (!TestNotNull(TEXT("Wall anchor"), Wall))
				{
					break;
				}
	...
	Describe("Room", [this]() {
		...
		LatentIt(TEXT("Try get closest point in room"), [this](const FDoneDelegate& Done) {
			...
			struct TestData
			{
				...
				TOptional<FUUID> ExpectedAnchorUUID;
			};
		...
		LatentIt(TEXT("Get key wall"), [this](const FDoneDelegate& Done) {
			...
			TestEqual(TEXT("Key wall anchor UUID is correct"), KeyWallAnchor->SpaceQueryResult.UUID, FUUID({ 0x93, 0x4C, 0xE7, 0x5D, 0x63, 0xF0, 0x85, 0x6A, 0x94, 0x38, 0xDA, 0xB3, 0xAD, 0xA9, 0x54, 0x09 }));
			Done.Execute();
		});
		...
		LatentIt(TEXT("Get largest surface"), [this](const FDoneDelegate& Done) {
			...
			TestEqual(TEXT("Anchor UUID is correct"), Anchor->SpaceQueryResult.UUID, FUUID({ 0xDE, 0x8D, 0xDD, 0xD9, 0x90, 0xAD, 0x5F, 0xCB, 0x8E, 0x7E, 0x23, 0x7E, 0x93, 0x8C, 0xB0, 0xD3 }));
			...
			TestEqual(TEXT("Anchor UUID is correct"), Anchor->SpaceQueryResult.UUID, FUUID({ 0x4F, 0x3D, 0x50, 0x69, 0x41, 0xFC, 0xDF, 0x2C, 0xDE, 0x52, 0x9B, 0x77, 0x8F, 0x7E, 0x2C, 0xA0 }));
		...
		LatentIt(TEXT("Parent/child relationship"), [this](const FDoneDelegate& Done) {
			...
			for (const auto& Anchor : Room->AllAnchors)
			{
				...
				{
					if (TestNotNull(TEXT("Screen has parent"), Anchor->ParentAnchor))
					{
	...
	Describe(TEXT("Anchor"), [this] {
		...
		LatentIt(TEXT("ProceduralMesh"), [this](const FDoneDelegate& Done) {
			...
			if (!TestNotNull(TEXT("Has Procedural Mesh Component"), ProceduralMeshComponent))
			{
				Done.Execute();
				return;
			}
