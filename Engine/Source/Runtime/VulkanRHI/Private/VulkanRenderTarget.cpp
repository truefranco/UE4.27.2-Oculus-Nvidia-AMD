...
void FVulkanCommandListContext::RHIEndRenderPass()
{
	...
	// MobileTonemapSubpass adds resolve attachment for non-MSAA tonemap output. No need to resolve in that case.
	if(!RenderPassInfo.bIsMSAA && ((RenderPassInfo.SubpassHint & ESubpassHint::MobileTonemapSubpass) == ESubpassHint::None))
	{
...
void FVulkanCommandListContext::RHINextSubpass()
{
	...
	ZeroVulkanStruct(SubpassBeginInfo, VK_STRUCTURE_TYPE_SUBPASS_BEGIN_INFO);
	...
	VulkanRHI::vkCmdNextSubpass2KHR(Cmd, &SubpassBeginInfo, &SubpassEndInfo);
}
	...
	, MultiViewCount(0)
{
	...
	for (int32 Index = 0; Index < RTInfo.NumColorRenderTargets; ++Index)
	{
		...
		if (RTView.Texture)
		{
			...
			VkAttachmentDescription2& CurrDesc = Desc[NumAttachmentDescriptions];
			CurrDesc.sType = VK_STRUCTURE_TYPE_ATTACHMENT_DESCRIPTION_2_KHR;
			CurrDesc.pNext = nullptr;
			...
			ColorReferences[NumColorAttachments].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
			...
			if (CurrDesc.samples > VK_SAMPLE_COUNT_1_BIT && bHasValidResolveAttachment)
			{
				...
				ResolveReferences[NumColorAttachments].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
	...
	if (RTInfo.DepthStencilRenderTarget.Texture)
	{
		...
		DepthStencilReference.sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
	...
	, MultiViewCount(RPInfo.MultiViewCount)
{
	...
	for (int32 Index = 0; Index < NumColorRenderTargets; ++Index)
	{
		...
		if ((CurrDesc.samples > VK_SAMPLE_COUNT_1_BIT || (RPInfo.SubpassHint & ESubpassHint::MobileTonemapSubpass) != ESubpassHint::None) && ColorEntry.ResolveTarget)
		{
			Desc[NumAttachmentDescriptions + 1] = Desc[NumAttachmentDescriptions];
			Desc[NumAttachmentDescriptions + 1].samples = VK_SAMPLE_COUNT_1_BIT;
			Desc[NumAttachmentDescriptions + 1].loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
	...
	, MultiViewCount(0)
{
	...
	for (uint32 Index = 0; Index < Initializer.RenderTargetsEnabled; ++Index)
	{
		...
		if (UEFormat != PF_Unknown)
		{
			...
			if (CurrDesc.samples > VK_SAMPLE_COUNT_1_BIT || (Initializer.SubpassHint & ESubpassHint::MobileTonemapSubpass) != ESubpassHint::None)
			{
				Desc[NumAttachmentDescriptions + 1] = Desc[NumAttachmentDescriptions];
				Desc[NumAttachmentDescriptions + 1].samples = VK_SAMPLE_COUNT_1_BIT;
				Desc[NumAttachmentDescriptions + 1].loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
