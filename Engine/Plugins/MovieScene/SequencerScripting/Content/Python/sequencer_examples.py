...
def create_level_sequence_with_mpc(mpc_asset_path, mpc_parameter_name, asset_name, package_path = '/Game/'):

	sequence = unreal.AssetToolsHelpers.get_asset_tools().create_asset(asset_name, package_path, unreal.LevelSequence, unreal.LevelSequenceFactoryNew())

	mpc = unreal.load_object(None, mpc_asset_path)

	mpc_track = sequence.add_master_track(unreal.MovieSceneMaterialParameterCollectionTrack)
	mpc_track.set_display_name(mpc.get_name())
	mpc_track.set_editor_property('MPC', mpc)
	
	mpc_section = mpc_track.add_section()
	mpc_section.set_start_frame_seconds(0)
	mpc_section.set_end_frame_seconds(5)
	
	mpc_section.add_scalar_parameter_key(mpc_parameter_name, unreal.FrameNumber(0), 0.24)
