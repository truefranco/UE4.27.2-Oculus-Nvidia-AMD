...
bool UPatternToolBuilder::CanBuildTool(const FToolBuilderState& SceneState) const
{
	...
	{
		if (Cast<UStaticMeshComponent>(Component) || Cast<USimpleDynamicMeshComponent>(Component))
		{
			ValidTargets++;
		}
	});
 ...
 */
class FPatternToolGizmoActorFactory : public FTransformGizmoActorFactory
{
public:
	FPatternToolGizmoActorFactory(UGizmoViewContext* GizmoViewContextIn)
	: FTransformGizmoActorFactory(GizmoViewContextIn)
	{
	 ...
	 */
	virtual ATransformGizmoActor* CreateNewGizmoActor(UWorld* World) const override
	{
...
public:
	...
	FVector3f StartTranslation = FVector3f::Zero();
	FVector3f EndTranslation = FVector3f::Zero();
	FVector3f TranslationJitterRange = FVector3f::Zero();
	bool bInterpolateTranslation = false;					// if false, only StartTranslation is used
	bool bJitterTranslation = false;

	FVector3f StartScale = FVector3f::One();
	FVector3f EndScale = FVector3f::One();
	FVector3f ScaleJitterRange = FVector3f::Zero();
...
public:
	...
	void AddPatternElement(FTransform3d NewTransform, double Alpha)
	{
	...
	void ApplyElementTransforms(FTransform3d& Transform, double Alpha)
	{
		...
		{
			Transform.SetScale(Transform.GetScale() * FVector3d(FVector3f::Lerp(StartScale, EndScale, Alpha)) );
		}
		else
		{
			Transform.SetScale( Transform.GetScale() * (FVector3d)StartScale );
		}

		if (bInterpolateTranslation)
		{
			Transform.SetTranslation( Transform.GetTranslation() + FVector3d(FVector3f::Lerp(StartTranslation, EndTranslation, Alpha)) );
		}
		else
		{
			Transform.SetTranslation( Transform.GetTranslation() + (FVector3d)StartTranslation );
		}
		...
		if (bJitterScale)
		{
			FVector3f ScaleJitter;
			FVector3f TransformScale = (FVector3f)Transform.GetScale();
			...
			Transform.SetScale((FVector3d)ScaleJitter );
		}


		if (bJitterTranslation)
		{
			FVector3f TranslationJitter;
			...
			Transform.SetTranslation( Transform.GetTranslation() + (FVector3d)TranslationJitter);
		}
	}
...
{
	double LineLength = StartFrame.Origin.Distance(EndFrame.Origin);
...
void FRadialPatternGenerator::UpdatePattern_CircleFill()
{
	...
	for (int32 k = 0; k < Iterations; ++k)
	{
		...
		if (bOriented)
		{
			FVector3d Axis = (PatternFrame.Origin - CenterFrame.Origin).Normalized();
...
void UPatternTool::Setup()
{
	...
	auto ApplyProportionalScale = [this](FVector3f& NewVector, FVector3f& CachedVector)
	{
		...
		FVector3f Difference = NewVector - CachedVector;
		int32 DifferenceMaxElementIndex = Difference.MaxAbsElementIndex();
		...
		double ScaleFactor = FMath::Max(NewVector[DifferenceMaxElementIndex] / CachedVector[DifferenceMaxElementIndex], UPatternTool_ScaleSettings::MinScale / CachedVector[CachedVector.MinElementIndex()]);
		
		NewVector = CachedVector * ScaleFactor;
		CachedVector = NewVector;
	};
	
	StartScaleWatcherIdx = ScaleSettings->WatchProperty((FVector3f&)ScaleSettings->StartScale, [this, &ApplyProportionalScale](const FVector3f& NewStartScale)
	{
		if (ScaleSettings->bProportional)
		{
			ApplyProportionalScale ((FVector3f&)ScaleSettings->StartScale, (FVector3f&)CachedStartScale);
	...
	EndScaleWatcherIdx = ScaleSettings->WatchProperty((FVector3f&)ScaleSettings->EndScale, [this, &ApplyProportionalScale](const FVector3f& NewEndScale)
	{
		if (ScaleSettings->bProportional)
		{
			ApplyProportionalScale((FVector3f&)ScaleSettings->EndScale, (FVector3f&)CachedEndScale);
	...
	JitterScaleWatcherIdx = ScaleSettings->WatchProperty((FVector3d&)ScaleSettings->Jitter, [this, &ApplyProportionalScale](const FVector3d& NewJitterScale)
	{
		if (ScaleSettings->bProportional)
		{
			ApplyProportionalScale((FVector3f&)ScaleSettings->Jitter, (FVector3f&)CachedJitterScale);
	...
	CurrentStartFrameWorld = FFrame3d((FTransform)Elements[0].SourceTransform);
...
void UPatternTool::RenderBoundingBoxes(IToolsContextRenderAPI* RenderAPI)
{
	...
	for (int32 ElemIdx = 0; ElemIdx < Elements.Num(); ++ElemIdx)
	{
		...
		{
			const FTransform3d RelativePositionTransform(FQuaterniond(RotationSettings->StartRotation), FVector::ZeroVector, ScaleSettings->StartScale);
			BoundingBoxVisualizer.PushTransform(FTransform((FVector)RelativePositionTransform.TransformVector((FVector)Element.RelativePosition)));
		}
	
		for (int32 k = 0; k < CurrentPattern.Num(); ++k)
		{
			BoundingBoxVisualizer.PushTransform(FTransform((FVector)CurrentPattern[k].GetTranslation()) * CurrentStartFrameWorld.ToFTransform());
...
void UPatternTool::InitializeElements()
{
	...
	for (int32 TargetIdx = 0; TargetIdx < NumElements; TargetIdx++)
	{
		...
		Element.SourceTransform.SetScale(FVector3d::One());
		...
		else if (USimpleDynamicMeshComponent* DynamicMeshComp = Cast<USimpleDynamicMeshComponent>(Element.SourceComponent))
		{
...
void UPatternTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
	...
	if (PropertySet == ScaleSettings && ScaleSettings->bProportional)
	{
		...
		{
			CachedStartScale = (FVector)ScaleSettings->StartScale;
			CachedEndScale = (FVector)ScaleSettings->EndScale;
			CachedJitterScale = ScaleSettings->Jitter;
...
{
	const FVector GizmoToToolOrigin = PatternGizmoProxy->GetTransform().GetLocation() - (FVector)CurrentStartFrameWorld.Origin;
	
	// Recompute extents using current shape, SingleAxis/SinglePlane, and PatternGizmo position
	switch (Settings->Shape)
	{
	case EPatternToolShape::Line:
		LinearSettings->Extent = GizmoToToolOrigin.ProjectOnTo((FVector)CurrentStartFrameWorld.GetAxis((int) Settings->SingleAxis)).Size();
	...
	case EPatternToolShape::Grid:
		...
		GridSettings->ExtentX = GizmoToToolOrigin.ProjectOnTo((FVector)CurrentStartFrameWorld.GetAxis(XIndex)).Size();
		if (GridSettings->bCenteredX) { GridSettings->ExtentX *= 2.0f; }

		GridSettings->ExtentY = GizmoToToolOrigin.ProjectOnTo((FVector)CurrentStartFrameWorld.GetAxis(YIndex)).Size();
	...
	case EPatternToolShape::Circle:
		RadialSettings->Radius = GizmoToToolOrigin.Size();	// This is actually the simplest case because of how conveniently a circle is defined
...
{
	FVector3d OffsetFromOrigin = FVector3d::Zero();
	...
	const FVector ProxyTranslation = (FVector)CurrentStartFrameWorld.Origin + FRotator(CurrentStartFrameWorld.Rotation).RotateVector((FVector)OffsetFromOrigin);
...
void UPatternTool::ReconstructTransformGizmos()
{
	if (bPatternToolThreeAxisTransformGizmoRegistered)
	{
		...
		PatternGizmo = Cast<UTransformGizmo>(GizmoManager->CreateGizmo(PatternToolThreeAxisTransformBuilderIdentifier, FString(), this));
...
void UPatternTool::GetPatternTransforms_Linear(TArray<FTransform3d>& TransformsOut)
{
...
void UPatternTool::GetPatternTransforms_Grid(TArray<FTransform3d>& TransformsOut)
{
	FLinearPatternGenerator Generator;
	InitializeGenerator(Generator, this);
...
void UPatternTool::GetPatternTransforms_Radial(TArray<FTransform3d>& TransformsOut)
{
...
void UPatternTool::UpdatePattern()
{
	...
	auto AddComponent = [this, &Pattern](int32 PatternItemIdx, const FPatternElement& Element, const FTransform3d& ElementTransform, FComponentSet& ElemComponents)
	{
		...
		if (Component != nullptr)
		{
			...
			ComputeWorldTransform(WorldTransform, (FTransform)ElementTransform, (FTransform)Pattern[PatternItemIdx]);
	...
	if (Settings->bRandomlyPickElements)
	{
		...
		for (int32 k = 0; k < NumPatternItems; ++k)
		{
			...
			FPatternElement& Element = Elements[ElemIdx];
			FTransform3d ElementTransform = Element.BaseRotateScale;
			FComponentSet& ElemComponents = PreviewComponents[ElemIdx];
...
void UPatternTool::ComputeWorldTransform(FTransform& OutWorldTransform, const FTransform& InElementTransform, const FTransform& InPatternTransform) const
{
	...
	if (Settings->bProjectElementsDown)
	{
		...
		const FRay WorldRay((FVector)OutWorldTransform.GetTranslation(), (FVector)ProjectionAxis);
		...
		if (ToolSceneQueriesUtil::FindNearestVisibleObjectHit(this, HitResult, WorldRay, &AllPreviewComponents, nullptr))
		{
			...
			OutWorldTransform.SetTranslation((FVector)OutWorldTransform.GetTranslation() + (FVector)ProjectionTranslation);
		}
	}
}
...
{
	FPatternElement& Element = Elements[ElemIdx];
	FTransform Transform = (FTransform)Element.BaseRotateScale;
	
	if (!BoundingBoxSettings->bIgnoreTransforms)
	{
		Transform = (FTransform) FTransform3d(FQuaterniond(RotationSettings->StartRotation),
			FVector::ZeroVector, ScaleSettings->StartScale) * Transform;
	}

	Element.PatternBounds = FAxisAlignedBox3d(Element.LocalBounds, (FTransform3d&)Transform);
}
...
USimpleDynamicMeshComponent* UPatternTool::GetPreviewDynamicMesh(const FPatternElement& Element)
{
	...
	if (FoundPool->Components.Num() > 0)
	{
		USimpleDynamicMeshComponent* FoundComponent = Cast<USimpleDynamicMeshComponent>(FoundPool->Components.Pop(false));
		check(FoundComponent != nullptr);
		return FoundComponent;
	}

	USimpleDynamicMeshComponent* DynamicMeshComp = NewObject<USimpleDynamicMeshComponent>(PreviewGeometry->GetActor());
...
void UPatternTool::ReturnDynamicMeshes(FPatternElement& Element, FComponentSet& ComponentSet)
{
	...
	for (UPrimitiveComponent* Component : ComponentSet.Components)
	{
		if (USimpleDynamicMeshComponent* DynamicMeshComponent = Cast<USimpleDynamicMeshComponent>(Component))
		{
...
void UPatternTool::EmitResults()
{
	...
	for (int32 ElemIdx = 0; ElemIdx < NumElements; ++ElemIdx)
	{
		...
		if (Element.SourceDynamicMesh != nullptr || bConvertToDynamic )
		{
			...
			auto EmitDynamicMeshActor = [this, &Element, ModelingSettings](FDynamicMesh3&& MoveMesh, FString BaseName, FTransform Transform)
			{
			...
			if (bSeparateActorPerItem)
			{
				for (int32 k = 0; k < NumPatternItems; ++k)
				{
					FTransform3d PatternTransform = CurrentPattern[k];
					FTransform WorldTransform;
