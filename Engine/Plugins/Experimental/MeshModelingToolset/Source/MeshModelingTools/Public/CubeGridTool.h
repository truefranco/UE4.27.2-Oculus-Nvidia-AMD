...
UCLASS()
class MESHMODELINGTOOLS_API UCubeGridToolBuilder : public UInteractiveToolWithToolTargetsBuilder
{
	GENERATED_BODY()
...
UCLASS()
class MESHMODELINGTOOLS_API UCubeGridToolProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
UCLASS()
class MESHMODELINGTOOLS_API UCubeGridToolActions : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
UCLASS()
class MESHMODELINGTOOLS_API UCubeGridDuringActivityActions : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
public:
	...
	struct FSelection
	{
		...
		FAxisAlignedBox3d StartBox; // Box delineating original selected face
	...
	// Used by undo/redo
	virtual void UpdateUsingMeshChange(const FDynamicMeshChange& MeshChange, bool bRevert);
...
protected:
	...
	UPROPERTY()
	UTransformGizmo* GridGizmo = nullptr;

	UPROPERTY()
	UDragAlignmentMechanic* GridGizmoAlignmentMechanic = nullptr;

	UPROPERTY()
	UTransformProxy* GridGizmoTransformProxy = nullptr;

	UPROPERTY()
	UPreviewGeometry* LineSets = nullptr;
	...
	UPROPERTY()
	ULocalSingleClickInputBehavior* CtrlMiddleClickBehavior = nullptr;

	UPROPERTY()
	ULocalClickDragInputBehavior* MiddleClickDragBehavior = nullptr;

	// Properties, etc
	UPROPERTY()
	UCubeGridToolProperties* Settings = nullptr;

	UPROPERTY()
	UCubeGridToolActions* ToolActions = nullptr;

	UPROPERTY()
	UCubeGridDuringActivityActions* DuringActivityActions = nullptr;

	UPROPERTY()
	UNewMeshMaterialProperties* MaterialProperties = nullptr;
	...
	void UpdateHoverLineSet(bool bHaveHover, const FAxisAlignedBox3d& HoveredBox);
	...
	FAxisAlignedBox3d HoveredSelectionBox;
	...
	TSharedPtr<FDynamicMeshAABBTree3, ESPMode::ThreadSafe> MeshSpatial;
	FTransform3d CurrentMeshTransform = FTransform3d::Identity();
	...
	TSharedPtr<const FDynamicMesh3, ESPMode::ThreadSafe> ComputeStartMesh;
	...
	*
	...
	FGeometrySet3 CornersGeometrySet;
