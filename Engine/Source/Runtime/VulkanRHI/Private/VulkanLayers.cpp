...
void FOptionalVulkanDeviceExtensions::Setup(const TArray<const ANSICHAR*>& DeviceExtensions)
{
	...
	HasQcomFragmentDensityMapOffset = HasExtension(DeviceExtensions, VK_QCOM_FRAGMENT_DENSITY_MAP_OFFSET_EXTENSION_NAME);
#endif

#if VULKAN_SUPPORTS_CREATE_RENDERPASS2
	HasKHRCreateRenderpass2 = HasExtension(DeviceExtensions, VK_KHR_CREATE_RENDERPASS_2_EXTENSION_NAME);
#endif
...
#if VULKAN_SUPPORTS_QCOM_RENDERPASS_SHADER_RESOLVE
	HasQcomRenderPassShaderResolve = HasExtension(DeviceExtensions, VK_QCOM_RENDER_PASS_SHADER_RESOLVE_EXTENSION_NAME );
#endif
}
