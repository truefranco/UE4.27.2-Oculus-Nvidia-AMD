...
#define USE_PERFETTO 0

#if ( STATS || ENABLE_STATNAMEDEVENTS ) && USE_PERFETTO
...
#define UE4_PERFETTO_CATEGORY_NAME_EVENTS   "com.epicgames.ue4.events"

PERFETTO_DEFINE_CATEGORIES(
	perfetto::Category(UE4_PERFETTO_CATEGORY_NAME_EVENTS)
	.SetDescription("Events from UE4"));
...
void FAndroidMisc::PlatformInit()
{
	...
	bPerfettoTraceActive = TRACE_EVENT_CATEGORY_ENABLED(UE4_PERFETTO_CATEGORY_NAME_EVENTS);
#endif
...
static void WriteTraceMarkerEvent(const ANSICHAR* Text, int32 TraceMarkerFileDescriptor)
{
	...
	{
#if USE_PERFETTO
		TRACE_EVENT_BEGIN(UE4_PERFETTO_CATEGORY_NAME_EVENTS, nullptr, [&](perfetto::EventContext ctx) {
...
void FAndroidMisc::BeginNamedEventStatic(const struct FColor& Color, const TCHAR* Text)
{
...
void FAndroidMisc::BeginNamedEventStatic(const struct FColor& Color, const ANSICHAR* Text)
{
	if (bPerfettoTraceActive)
	{
#if USE_PERFETTO
		TRACE_EVENT_BEGIN(UE4_PERFETTO_CATEGORY_NAME_EVENTS, perfetto::StaticString(Text));
#endif
	}
	else
	{
...
void FAndroidMisc::EndNamedEvent()
{
	...
	{
#if USE_PERFETTO
		TRACE_EVENT_END(UE4_PERFETTO_CATEGORY_NAME_EVENTS);
#endif
	} 
	else if (bUseNativeSystrace)
	{
