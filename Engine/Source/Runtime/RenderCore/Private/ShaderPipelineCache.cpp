...
bool FShaderPipelineCache::Precompile(FRHICommandListImmediate& RHICmdList, EShaderPlatform Platform, FPipelineCacheFileFormatPSO const& PSO)
{
	...
	#endif
			auto BlendState = PipelineStateCache::GetAndOrCreateBlendState(PSO.GraphicsDesc.BlendState);
			GraphicsInitializer.BlendState = BlendState;
			
			auto RasterState = PipelineStateCache::GetAndOrCreateRasterizerState(PSO.GraphicsDesc.RasterizerState);
			GraphicsInitializer.RasterizerState = RasterState;
			
			auto DepthState = PipelineStateCache::GetAndOrCreateDepthStencilState(PSO.GraphicsDesc.DepthStencilState);
