...
inline bool RHIDiscardsMSAATextures(const FStaticShaderPlatform Platform)
{
	// Vulkan mobile requires MSAA attachments, but for TBDR reasons will never resolve those contents to sysmem
	return RHISupportsSeparateMSAAAndResolveTextures(Platform) && IsVulkanMobilePlatform(Platform);
}
