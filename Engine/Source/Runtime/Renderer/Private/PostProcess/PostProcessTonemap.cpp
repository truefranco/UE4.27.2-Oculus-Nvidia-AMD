...
class FMobileTonemapSubpassPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_EX(FVector4, ColorShadow_Tint1, EShaderPrecisionModifier::Half)
		SHADER_PARAMETER_EX(FVector4, ColorShadow_Tint2, EShaderPrecisionModifier::Half)
		SHADER_PARAMETER_TEXTURE(, ColorGradingLUT)
		SHADER_PARAMETER_SAMPLER(SamplerState, ColorGradingLUTSampler)
	...
	static FPermutationDomain BuildPermutationVector(const FViewInfo& View, uint32 MsaaSamples, bool bColorLUT)
	{
		...
		if (bColorLUT)
		{
			SubpassPermutationVector.Set<FTonemapperColorMatrixDim>(false);
			SubpassPermutationVector.Set<FTonemapperShadowTintDim>(false);
			SubpassPermutationVector.Set<FTonemapperContrastDim>(false);
		}
		else
		{
				FVector MixerR(Settings.FilmChannelMixerRed);
				...
				if (
					...
					((MixerB - FVector(0.0f, 0.0f, 1.0f)).GetAbsMax() != 0.0f))
				{
					SubpassPermutationVector.Set<FTonemapperColorMatrixDim>(true);
				}
			
			SubpassPermutationVector.Set<FTonemapperShadowTintDim>(Settings.FilmShadowTintAmount > 0.0f);
			SubpassPermutationVector.Set<FTonemapperContrastDim>(Settings.FilmContrast > 0.0f);
		}
...
void AddMobileTonemapperSubpass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FMobileTonemapperInputs& Inputs)
{
	// Part of scene rendering pass
	check(GraphBuilder.RHICmdList.IsInsideRenderPass());

	SCOPED_DRAW_EVENT(GraphBuilder.RHICmdList, MobileTonemapSubpass);

	FSceneRenderTargets& SceneContext = FSceneRenderTargets::Get(GraphBuilder.RHICmdList);
#if UE_EDITOR
	FUniformBufferRHIRef PassUniformBuffer = CreateMobileSceneTextureUniformBuffer(GraphBuilder.RHICmdList, EMobileSceneTextureSetupMode::SceneColor);
	...
	FMobileTonemapSubpassPS::FParameters* PSShaderParameters = GraphBuilder.AllocParameters<FMobileTonemapSubpassPS::FParameters>();
	...
	SetShaderParameters(GraphBuilder.RHICmdList, VertexShader, VertexShader.GetVertexShader(), *VSShaderParameters);
	SetShaderParameters(GraphBuilder.RHICmdList, PixelShader, PixelShader.GetPixelShader(), *PSShaderParameters);

	GraphBuilder.RHICmdList.SetViewport(0, 0, 0.0f, TargetSize.X, TargetSize.Y, 1.0f);
