...
void FVulkanCmdBuffer::EndRenderPass()
{
	...
	{
		const VkExtent2D& Granularity = Device->GetFragmentDensityMapOffsetProperties().fragmentDensityOffsetGranularity;

		Offsets[0].x = GVulkanQcomFragmentDensityMapOffsets[0].X / Granularity.width * Granularity.width;
		Offsets[0].y = GVulkanQcomFragmentDensityMapOffsets[0].Y / Granularity.height * Granularity.height;
		Offsets[1].x = GVulkanQcomFragmentDensityMapOffsets[1].X / Granularity.width * Granularity.width;
		Offsets[1].y = GVulkanQcomFragmentDensityMapOffsets[1].Y / Granularity.height * Granularity.height;
...
void FVulkanCmdBuffer::BeginRenderPass(const FVulkanRenderTargetLayout& Layout, FVulkanRenderPass* RenderPass, FVulkanFramebuffer* Framebuffer, const VkClearValue* AttachmentClearValues)
{
	...
	RenderPassBeginInfo.clearValueCount = Layout.GetNumUsedClearValues();
	...
	if (QCOMTransform != VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR)
	{
		...
		RenderPassBeginInfo.pNext = &RPTransformBeginInfoQCOM;
	}
#endif
	...
	VulkanRHI::vkCmdBeginRenderPass2KHR(CommandBufferHandle, &RenderPassBeginInfo, &SubpassBeginInfo);
