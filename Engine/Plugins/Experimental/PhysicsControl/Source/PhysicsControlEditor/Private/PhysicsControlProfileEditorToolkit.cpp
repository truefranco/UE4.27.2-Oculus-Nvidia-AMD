	...
	UPhysicsControlProfileAsset*    InPhysicsControlProfileAsset)
{
	...
	// Create Persona toolkit
	{
		...
		TSharedRef<IAssetFamily> AssetFamily = PersonaModule.CreatePersonaAssetFamily(InPhysicsControlProfileAsset);
		AssetFamily->RecordAssetOpened(FAssetData(InPhysicsControlProfileAsset));
	}
