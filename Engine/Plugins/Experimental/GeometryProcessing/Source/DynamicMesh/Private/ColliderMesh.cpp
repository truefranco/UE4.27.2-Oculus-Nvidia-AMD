...
bool FColliderMesh::FindNearestHitTriangle(const FRay3d& Ray, double& RayParameterOut, int& HitTriangleIDOut) const
{
	if ( AABBTree.IsValid(false) )
	{
		return AABBTree.FindNearestHitTriangle(Ray, RayParameterOut, HitTriangleIDOut);
	}
	return false;
}
