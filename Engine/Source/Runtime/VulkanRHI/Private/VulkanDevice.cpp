...
void FVulkanDevice::CreateDevice()
{
	...
	if (OptionalDeviceExtensions.HasEXTFragmentDensityMap)
	{
		...
		DeviceFDMFeatures.fragmentDensityMap = GetFragmentDensityMapFeatures().fragmentDensityMap ? VK_TRUE : VK_FALSE;
	...
	VkPhysicalDeviceFragmentDensityMapOffsetFeaturesQCOM DeviceFDMOffsetFeatures;
	if (OptionalDeviceExtensions.HasQcomFragmentDensityMapOffset)
	{
		ZeroVulkanStruct(DeviceFDMOffsetFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_OFFSET_FEATURES_QCOM);
		DeviceFDMOffsetFeatures.fragmentDensityMapOffset = VK_TRUE;
		DeviceFDMOffsetFeatures.pNext = (void*)DeviceInfo.pNext;
		DeviceInfo.pNext = &DeviceFDMOffsetFeatures;
	}
#endif
...
#if VULKAN_SUPPORTS_FRAGMENT_DENSITY_MAP_OFFSET
		if (GetOptionalExtensions().HasQcomFragmentDensityMapOffset)
		{
			*NextPropsAddr = &FragmentDensityMapOffsetProperties;
			NextPropsAddr = &FragmentDensityMapOffsetProperties.pNext;
		}
#endif
...
void FVulkanDevice::InitGPU(int32 DeviceIndex)
{
	...
	if (RHI->GetOptionalExtensions().HasKHRGetPhysicalDeviceProperties2)
	{
		...
		if (GetOptionalExtensions().HasQcomFragmentDensityMapOffset)
		{
			*NextPropsAddr = &FragmentDensityMapOffsetFeatures;
			NextPropsAddr = &FragmentDensityMapOffsetFeatures.pNext;
