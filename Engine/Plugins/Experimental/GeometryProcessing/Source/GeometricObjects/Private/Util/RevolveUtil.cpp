	...
	const FVector3d& RevolutionAxisDirection, double Tolerance, TSet<int32>& ProfileVerticesToWeldOut)
{
	...
	for (int32 ProfileIndex = 0; ProfileIndex < ProfileCurve.Num(); ++ProfileIndex)
	{
		...
		double dist = VectorToPoint.DistanceSquared(VectorProjectedToAxis);
	...
	const FVector3d& RevolutionAxisDirection, bool bProfileCurveIsClosed)
{
	...
	double PointDistance = (ProfileCurve[MinAlongAxisIndex] - RevolutionAxisOrigin).Distance(MinAlongAxis * RevolutionAxisDirection);
	...
	double PreviousDistance = PreviousWrtAxis.Distance(PreviousAlongAxis * RevolutionAxisDirection);
