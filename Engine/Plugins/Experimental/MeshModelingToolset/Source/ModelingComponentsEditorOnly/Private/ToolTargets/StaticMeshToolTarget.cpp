	...
	EMeshLODIdentifier RequestedEditingLOD)
{
	...
	if (ensure(StaticMeshIn != nullptr))
	{
		...
		{
			ValidEditingLOD = StaticMeshIn->IsMeshDescriptionValid(0) ? EMeshLODIdentifier::HiResSource : EMeshLODIdentifier::LOD0;
		}
...
TArray<EMeshLODIdentifier> UStaticMeshToolTarget::GetAvailableLODs(const UStaticMesh* InStaticMesh, bool bSkipAutoGenerated)
{
	...
	if (InStaticMesh->IsMeshDescriptionValid(0))
	{
	...
	for (int32 LOD = 0; LOD < NumExistingLOD; ++LOD)
	{
		if (bSkipAutoGenerated)
		{
			...
			if (!InStaticMesh->GetSourceModel(LOD).MeshDescription && !InStaticMesh->GetSourceModel(LOD).MeshDescriptionBulkData)
			{
				continue;
			}
		}
...
{
	if (!StaticMeshIn || StaticMeshIn->IsUnreachable() || !StaticMeshIn->IsValidLowLevel())
	{
		return false;
	}
	...
	{
		if (StaticMeshIn->IsMeshDescriptionValid(0) == false)
		{
			return false;
		}
	}
...
bool UStaticMeshToolTarget::HasNonGeneratedLOD(const UStaticMesh* StaticMeshIn, EMeshLODIdentifier MeshLOD)
{
	...
	{
		return StaticMeshIn->IsMeshDescriptionValid(0);
	}
	const int32 NumExistingLOD = FMath::Min(8, StaticMeshIn->GetNumSourceModels());
	if (MeshLOD == EMeshLODIdentifier::MaxQuality)
	{
		const bool bHasHiRes = StaticMeshIn->IsMeshDescriptionValid(0);
		const bool bHasLOD0 = (NumExistingLOD > 0) ? StaticMeshIn->IsMeshDescriptionValid(0) : false;
	...
	else
	{
		...
		return StaticMeshIn->IsMeshDescriptionValid(LODint);
	}
}
...
void UStaticMeshToolTarget::CommitMeshDescription(UStaticMesh* StaticMeshIn, const FCommitter& Committer, EMeshLODIdentifier EditingLODIn)
{
	...
	{
		UE_LOG(LogDynamicMesh, Warning, TEXT("UStaticMeshToolTarget::CommitMeshDescription: invalid Target LOD, must specify explicit LOD"));
		return;
	}
	if (StaticMeshIn->GetPathName().StartsWith(TEXT("/Engine/")))
	{
	...
	if (EditingLODIn == EMeshLODIdentifier::HiResSource)
	{
		UpdateMeshDescription = StaticMeshIn->GetSourceModel(0).MeshDescription.Get();
		if (UpdateMeshDescription == nullptr)
		{
			UpdateMeshDescription = StaticMeshIn->CreateMeshDescription(0);
		}
	}
	else
	{
	...
	if (EditingLODIn == EMeshLODIdentifier::HiResSource)
	{
		//FStaticMeshSourceModel& SourceModel = StaticMeshIn->GetSourceModel(0);
		check(StaticMeshIn->GetSourceModel(0).MeshDescriptionBulkData != nullptr);
		StaticMeshIn->Modify();
	}
	else
	{
		check(StaticMeshIn->GetSourceModel((int32)EditingLODIn).MeshDescriptionBulkData != nullptr);
		StaticMeshIn->Modify(true);
	}
	...
	if (EditingLODIn == EMeshLODIdentifier::HiResSource)
	{
		StaticMeshIn->CommitMeshDescription(0);
	}
	else
	{
		...
		ThisSourceModel.ReductionSettings.MaxDeviation = 0.0f;
...
void UStaticMeshToolTarget::CommitDynamicMesh(const FDynamicMesh3& Mesh, const FDynamicMeshCommitInfo& CommitInfo)
{
	...
	UE::DyVIA::CommitDynamicMeshViaMeshDescription(MoveTemp(CurrentMeshDescription), *this, Mesh, CommitInfo);
}
