...
public:
	...
	virtual void CommitDynamicMesh(const FDynamicMesh3& Mesh)
	{
		FDynamicMeshCommitInfo CommitInfo;
		CommitDynamicMesh(Mesh, CommitInfo);
	};
	virtual void CommitDynamicMesh(const FDynamicMesh3& Mesh, const FDynamicMeshCommitInfo& CommitInfo) = 0;
};
