	...
	bool						bSourceContainsMetaDataOnly)
{
	...
	if (ShaderInput.ExtraSettings.OfflineCompilerPath.Len() > 0)
	{
		...
		{
			CompileShaderOffline(ShaderInput, ShaderOutput, (const ANSICHAR*)Spirv.Data.GetData(), Spirv.Data.Num() * sizeof(uint32), true, (const ANSICHAR*)(Spirv.Data.GetData() + Spirv.OffsetToMainName));
		}
	}
