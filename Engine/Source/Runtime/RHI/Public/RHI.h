...
public:
	...
	,	bUseAlphaToCoverage(bInUseAlphaToCoverage)
	{
		...
		while (RenderTargetIndex < NumRenderTargets)
		{
			RenderTargets[RenderTargetIndex] = InRenderTargetBlendStates[RenderTargetIndex];
			++RenderTargetIndex;
		}
		// make sure unused slots are also written because it effects the PSO hash
		while (RenderTargetIndex < MaxSimultaneousRenderTargets)
		{
			RenderTargets[RenderTargetIndex++] = FRenderTarget();
		}
	}
