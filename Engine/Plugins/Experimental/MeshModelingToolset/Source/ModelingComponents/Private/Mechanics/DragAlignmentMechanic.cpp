...
void FDragAlignmentBase::AddToGizmo(UTransformGizmo* TransformGizmo, const TArray<const UPrimitiveComponent*>* ComponentsToIgnoreInAlignment, 
...
void FDragAlignmentBase::SetupInternal()
{
		...
		const TArray<const UPrimitiveComponent*>* InvisibleComponentsToInclude)
	{
		...
		else if (ToolSceneQueriesUtil::FindNearestVisibleObjectHit(TargetWorld, HitResult, WorldRay))
		{
			HitResult.Item = TRIANGLE_HIT_TYPE_ID;
			bSuccess = true;
		}
...
void UDragAlignmentInteraction::Setup()
{
	SetupInternal();
}
