...
void FGfxPipelineDesc::FRenderTargets::FAttachmentRef::ReadFrom(const VkAttachmentReference2& InState)
{
	Type =			(uint32)InState.sType;
	Attachment =	InState.attachment;
	Layout =		(uint64)InState.layout;
}

void FGfxPipelineDesc::FRenderTargets::FAttachmentRef::WriteInto(VkAttachmentReference2& Out) const
{
	Out.sType =			(VkStructureType)Type;
	Out.attachment =	Attachment;
	Out.layout =		(VkImageLayout)Layout;
}
...
void FGfxPipelineDesc::FRenderTargets::FAttachmentDesc::ReadFrom(const VkAttachmentDescription2 &InState)
{
	Type =				(uint32)InState.sType;
	Format =			(uint32)InState.format;
	Flags =				(uint8)InState.flags;
...
void FGfxPipelineDesc::FRenderTargets::FAttachmentDesc::WriteInto(VkAttachmentDescription2& Out) const
{
	Out.sType =				(VkStructureType)Type;
	Out.format =			(VkFormat)Format;
	Out.flags =				Flags;
...
void FGfxPipelineDesc::FRenderTargets::ReadFrom(const FVulkanRenderTargetLayout& RTLayout)
{
	...
	auto CopyAttachmentRefs = [&](TArray<FGfxPipelineDesc::FRenderTargets::FAttachmentRef>& Dest, const VkAttachmentReference2* Source, uint32 Count)
	{
...
void FGfxPipelineDesc::FRenderTargets::WriteInto(FVulkanRenderTargetLayout& Out) const
{
	...
	auto CopyAttachmentRefs = [&](const TArray<FGfxPipelineDesc::FRenderTargets::FAttachmentRef>& Source, VkAttachmentReference2* Dest, uint32 Count)
	{
...
void FVulkanPipelineStateCacheManager::CreateGfxEntry(const FGraphicsPipelineStateInitializer& PSOInitializer, FVulkanDescriptorSetsLayoutInfo& DescriptorSetLayoutInfo, FGfxPipelineDesc* Desc)
{
	...
	if (GIsEditor && ((PSOInitializer.SubpassHint & ESubpassHint::MobileTonemapSubpass) != ESubpassHint::None) && PSOInitializer.SubpassIndex >= 2)
	{
		// GBuffer attachments are not used as output in a shading sub-pass
		// Only SceneColor is used as a color attachment
		NumRenderTargets = 1;
