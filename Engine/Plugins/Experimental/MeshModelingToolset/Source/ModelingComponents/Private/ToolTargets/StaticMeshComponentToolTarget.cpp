	...
	if (ensure(IsValid()))
	{
		...
		if (UStaticMesh* StaticMesh = Cast<UStaticMeshComponent>(Component)->GetStaticMesh())
		{
			...
			FoundMeshDescription = (UseLOD == EMeshLODIdentifier::HiResSource) ?
				StaticMesh->GetSourceModel(0).MeshDescription.Get() : StaticMesh->GetMeshDescription((int32)UseLOD);
		}
	...
	if (ensure(IsValid()))
	{
		if (UStaticMesh* StaticMesh = Cast<UStaticMeshComponent>(Component)->GetStaticMesh())
		{
			...
			if (UseLOD == EMeshLODIdentifier::HiResSource )
			{
				...
				{
					FMeshDescription MeshDescriptionCopy = *StaticMesh->GetMeshDescription(0);
...
{
	return UE::DyVIA::GetDynamicMeshViaMeshDescription(*this);
}
...
void UStaticMeshComponentToolTarget::CommitDynamicMesh(const FDynamicMesh3& Mesh, const FDynamicMeshCommitInfo& CommitInfo)
{
	...
	{
		UE::DyVIA::CommitDynamicMeshViaMeshDescription(FMeshDescription(*CurrentMeshDescription), *this, Mesh, CommitInfo);
	}
}
