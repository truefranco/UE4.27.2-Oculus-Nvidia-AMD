...
namespace CubeGridToolLocals
{
	...
	{
	public:
		FCubeGridToolMeshChange(TUniquePtr<FDynamicMeshChange> MeshChangeIn)
		...
		double DepthBias, const FVector3d* CornerVector = nullptr)
	{
		...
		LineSet.AddLine((FVector)GridMin, (FVector)Corner1, Color, Thickness, DepthBias);
		LineSet.AddLine((FVector)GridMin, (FVector)Corner2, Color, Thickness, DepthBias);
		LineSet.AddLine((FVector)GridMax, (FVector)Corner1, Color, Thickness, DepthBias);
		LineSet.AddLine((FVector)GridMax, (FVector)Corner2, Color, Thickness, DepthBias);

		if (CornerVector)
		{
			LineSet.AddLine((FVector)GridMin, (FVector)GridMin + (FVector)*CornerVector, Color, Thickness, DepthBias);
			LineSet.AddLine((FVector)GridMax, (FVector)GridMax + (FVector)*CornerVector, Color, Thickness, DepthBias);
			LineSet.AddLine((FVector)Corner1, (FVector)Corner1 + (FVector)*CornerVector, Color, Thickness, DepthBias);
			LineSet.AddLine((FVector)Corner2, (FVector)Corner2 + (FVector)*CornerVector, Color, Thickness, DepthBias);
		}
	}
...
void UCubeGridTool::Setup()
{
	...
	MaterialProperties->WatchProperty(MaterialProperties->bWireframe,
		[this](float UVScale) {
	...
	GridFrameOriginWatcherIdx = Settings->WatchProperty(Settings->GridFrameOrigin,
		[this](FVector) {
			FTransform Transform(Settings->GridFrameOrientation, Settings->GridFrameOrigin, FVector::OneVector);
...
{
	ComputeStartMesh = MakeShared<FDynamicMesh3, ESPMode::ThreadSafe>(*CurrentMesh);
}
...
void UCubeGridTool::Render(IToolsContextRenderAPI* RenderAPI)
{
	...
	if (Mode == EMode::Corner)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			FVector WorldPosition = (FVector)CubeGrid->GetFrame().FromFramePoint(OrientedBox.GetCorner(i));
...
void UCubeGridTool::OnCtrlMiddleClick(const FInputDeviceRay& ClickPos)
{
	...
	FRay GizmoSpaceRay((FVector)GridSpaceRayOrigin, (FVector)GridSpaceRayDirection);

	double MinDistSquared = GizmoSpaceRay.DistSquared((FVector)Corners[0]);
	int32 ClosestCornerIndex = 0;
	for (int32 i = 1; i < 4; ++i)
	{
		double DistSquared = GizmoSpaceRay.DistSquared((FVector)Corners[i]);
	...
	UpdateGridGizmo(FTransform(Settings->GridFrameOrientation, (FVector)CubeGrid->ToWorldPoint(Corners[ClosestCornerIndex]), FVector::OneVector));
...
bool UCubeGridTool::GetHitGridFace(const FRay& WorldRay, FCubeGrid::FCubeFace& FaceOut)
{
	...
	bool bHitPlane;
	FVector IntersectionPoint;
	GizmoMath::RayPlaneIntersectionPoint((FVector)CubeGrid->GetFrame().Origin, (FVector)CubeGrid->GetFrame().Z(),
	...
	if (MeshSpatial)
	{
		...
		{
			double HitT = WorldRay.GetParameter((FVector)CurrentMeshTransform.TransformPosition(LocalRay.PointAt(LocalHitT)));
			if (HitT < BestHitT)
			{
				BestHitT = HitT;
				ConvertToFaceHit(*CubeGrid, Settings->FaceSelectionMode,
...
void UCubeGridTool::OnClickPress(const FInputDeviceRay& PressPos)
{
	...
	if (bMouseDragShouldPushPull)
	{
		...
		if (bHaveSelection)
		{
			...
			FDistLine3Ray3d DistanceCalculator(FLine3d(DragProjectionAxis.Origin, DragProjectionAxis.Direction), (FRay3d)PressPos.WorldRay);
...
void UCubeGridTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	...
	if (MouseState == EMouseState::DraggingExtrudeDistance)
	{
		...
		FDistLine3Ray3d DistanceCalculator(
			FLine3d(DragProjectionAxis.Origin, DragProjectionAxis.Direction), (FRay3d)DragPos.WorldRay);
		DistanceCalculator.ComputeResult();
...
void UCubeGridTool::UpdateHoverLineSet(bool bNewHaveHover, const FAxisAlignedBox3d& NewHoveredBox)
{
	using namespace CubeGridToolLocals;
	...
	if (Mode == EMode::Corner && CurrentExtrudeAmount < 0)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (!CornerWelded[i])
			{
				...
				LineSet->AddLine((FVector)CurrentCorner, (FVector)UpCorner, CornerModeWireframeColor,
			...
			LineSet->AddLine((FVector)CurrentCorner, (FVector)NextCorner, CornerModeWireframeColor,
				CornerModeWireframeThickness, CornerModeWireframeDepthBias);
		}
		...
		LineSet->AddLine((FVector)DiagCorner1, (FVector)DiagCorner2, CornerModeWireframeColor,
			CornerModeWireframeThickness, CornerModeWireframeDepthBias);
	}
}
