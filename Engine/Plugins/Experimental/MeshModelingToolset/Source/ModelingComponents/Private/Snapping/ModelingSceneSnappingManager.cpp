...
bool UModelingSceneSnappingManager::IsComponentTypeSupported(const UPrimitiveComponent* Component) const
{
	...
	return ( Cast<UStaticMeshComponent>(Component) != nullptr ||
			 ...
			 Cast<USimpleDynamicMeshComponent>(Component) != nullptr );
}
...
void UModelingSceneSnappingManager::OnComponentAdded(UPrimitiveComponent* Component)
{
	...
	if (USimpleDynamicMeshComponent* DynamicMeshComponent = Cast<USimpleDynamicMeshComponent>(Component))
	{
		...
		DynamicMeshComponents.Add(DynamicMeshComponent, TWeakObjectPtr<USimpleDynamicMeshComponent>(DynamicMeshComponent) );
	}
...
void UModelingSceneSnappingManager::OnComponentRemoved(UPrimitiveComponent* Component)
{
	...
	TWeakObjectPtr<USimpleDynamicMeshComponent>* FoundDynamicMesh = DynamicMeshComponents.Find(Component);
	if (FoundDynamicMesh != nullptr)
	{
		if (FoundDynamicMesh->IsValid())
		{
			USimpleDynamicMeshComponent* DynamicMeshComponent = FoundDynamicMesh->Get();
...
{
	if (Cast<USimpleDynamicMeshComponent>(Object))
	{
...
void UModelingSceneSnappingManager::HandleDynamicMeshModifiedDelegate(USimpleDynamicMeshComponent* Component)
{
...
void UModelingSceneSnappingManager::UnPauseSceneGeometryUpdates(bool bImmediateRebuilds)
{
	...
	{
		for (USimpleDynamicMeshComponent* Component : PendingModifiedDynamicMeshes)
		{
...
static bool FindNearestVisibleObjectHit_Internal(
	...
	FSceneGeometrySpatialCache* SpatialCache)
{
...
bool GetComponentHitTriangle_Internal(FHitResult HitResult, VectorType* TriVertices)
{
	...
	const int32 MinLODIdx = StaticMesh->GetMinLOD().GetValue();
...
bool UModelingSceneSnappingManager::ExecuteSceneHitQuery(const FSceneHitQueryRequest& Request, FSceneHitQueryResult& ResultOut) const
{
	...
	bool bHitSceneCache = SpatialCache->FindNearestHit((FRay)WorldRay, HitPoint, HitIdentifier, &Request.VisibilityFilter );
	if (bHitSceneCache)
	{
		FVector3d A, B, C;
		...
		ResultOut.Position = (FVector)HitPoint.WorldPoint;
		ResultOut.Normal = (FVector)VectorUtil::Normal(A, B, C);
		ResultOut.TriVertices[0] = (FVector)A;
		ResultOut.TriVertices[1] = (FVector)B;
		ResultOut.TriVertices[2] = (FVector)C;
	...
	if (bHitWorld && HitResult.Distance < MinCurrentHitDist )
	{
		...
		ResultOut.Position = HitResult.Location;
		ResultOut.Normal = HitResult.Normal;
		ResultOut.HitResult = HitResult;
		if (Request.bWantHitGeometryInfo)
		{ 
			GetComponentHitTriangle_Internal<FVector>(HitResult, ResultOut.TriVertices);
		}
	}
...
bool UModelingSceneSnappingManager::ExecuteSceneSnapQueryPosition(const FSceneSnapQueryRequest& Request, TArray<FSceneSnapQueryResult>& Results) const
{
	...
	if ((Request.TargetTypes & ESceneSnapQueryTargetType::Grid) != ESceneSnapQueryTargetType::None)
	{
		...
		if (SnappingConfig.bEnablePositionGridSnapping)
		{
			...
			SnapResult.Position.X = VectorUtil::SnapToIncrement(Request.Position.X, GridSize.X);
			SnapResult.Position.Y = VectorUtil::SnapToIncrement(Request.Position.Y, GridSize.Y);
			SnapResult.Position.Z = VectorUtil::SnapToIncrement(Request.Position.Z, GridSize.Z);
	...
	auto TrySnapToVertex = [](const FSceneSnapQueryRequest& RequestIn, const FRay3d& WorldRay, FSceneSnapQueryResult& SnapResult, double& SmallestSnapAngle)
	{
		if ((RequestIn.TargetTypes & ESceneSnapQueryTargetType::MeshVertex) != ESceneSnapQueryTargetType::None)
		{
			for (int j = 0; j < 3; ++j)
			{
				double VisualAngle = VectorUtil::OpeningAngleD((FVector3d)RequestIn.Position, (FVector3d)SnapResult.TriVertices[j], WorldRay.Origin);
	...
	auto TrySnapToEdge = [](const FSceneSnapQueryRequest& RequestIn, const FRay3d& WorldRay, FSceneSnapQueryResult& SnapResult, double& SmallestSnapAngle)
	{
			  ...
			  (SnapResult.TargetType != ESceneSnapQueryTargetType::MeshVertex) )
		{
			for (int j = 0; j < 3; ++j)
			{
				FSegment3d Segment(SnapResult.TriVertices[j], SnapResult.TriVertices[(j+1)%3]);
				FVector3d EdgeNearestPt = Segment.NearestPoint(RequestIn.Position);
				double VisualAngle = VectorUtil::OpeningAngleD((FVector3d)RequestIn.Position, EdgeNearestPt, WorldRay.Origin);
				if (VisualAngle < SmallestSnapAngle )
				{
					SmallestSnapAngle = VisualAngle;
					SnapResult.Position = (FVector)EdgeNearestPt;
	...
	FRay3d WorldRay(RayStart, RayDirection.Normalized());
	...
	FSceneGeometryPoint HitPoint;
	FSceneGeometryID HitIdentifier;
	bool bHitSceneCache = SpatialCache->FindNearestHit((FRay)WorldRay, HitPoint, HitIdentifier, &TmpFilter);
	...
	if (bHitSceneCache)
	{
		...
		double VisualAngle = VectorUtil::OpeningAngleD((FVector3d)Request.Position, HitPoint.WorldPoint, RayStart);
	...
	{
		// is this check necessary? isn't this always going to be zero for a scene hit??
		double VisualAngle = VectorUtil::OpeningAngleD((FVector3d)Request.Position, (FVector3d)HitResult.ImpactPoint, RayStart);
		if (VisualAngle < (double)Request.VisualAngleThresholdDegrees)
		{
			FSceneSnapQueryResult SnapResult;
			if ( GetComponentHitTriangle_Internal<FVector>(HitResult, SnapResult.TriVertices) )
			{
				...
				if (SmallestAngle < (double)Request.VisualAngleThresholdDegrees)
				{
					SnapResult.TargetActor = HitResult.GetActor();
