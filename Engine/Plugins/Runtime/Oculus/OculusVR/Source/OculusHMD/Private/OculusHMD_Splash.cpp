...
void FSplash::RenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList)
{
...

	{
		...
		while(LayerIndex < XLayers.Num() && LayerIndex_RenderThread < Layers_RenderThread.Num())
		{
			...
			{
				XLayers[LayerIndex++]->Initialize_RenderThread(XSettings.Get(), CustomPresent, &OculusHMD->DeferredDeletion, RHICmdList);
			}
			else if (LayerIdA > LayerIdB)
			{
				OculusHMD->DeferredDeletion.AddLayerToDeferredDeletionQueue(Layers_RenderThread[LayerIndex_RenderThread++]);
			}
			else
			{
				XLayers[LayerIndex++]->Initialize_RenderThread(XSettings.Get(), CustomPresent, &OculusHMD->DeferredDeletion, RHICmdList, Layers_RenderThread[LayerIndex_RenderThread++].Get());
			}
		}
