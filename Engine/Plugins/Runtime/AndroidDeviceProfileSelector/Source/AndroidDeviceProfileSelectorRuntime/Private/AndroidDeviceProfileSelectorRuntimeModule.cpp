...
FString const FAndroidDeviceProfileSelectorRuntimeModule::GetRuntimeDeviceProfileName()
{
	...
	if (ProfileName.IsEmpty())
	{
		...
		UE_LOG(LogAndroid, Log, TEXT("  HMDSystemName: %s"), *HMDRequestedProfileName);
		...
		else
		{
			...
			ProfileName = FAndroidDeviceProfileSelector::FindMatchingProfile(GPUFamily, GLVersion, AndroidVersion, DeviceMake, DeviceModel, DeviceBuildNumber, VulkanAvailable, VulkanVersion, UsingHoudini, Hardware, Chipset, HMDRequestedProfileName, ProfileName);
			UE_LOG(LogAndroid, Log, TEXT("Selected Device Profile: [%s]"), *ProfileName);
		}
	}
