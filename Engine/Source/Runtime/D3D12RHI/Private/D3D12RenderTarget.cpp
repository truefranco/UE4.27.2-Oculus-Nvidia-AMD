...
void FD3D12CommandContext::RHICopyToResolveTarget(FRHITexture* SourceTextureRHI, FRHITexture* DestTextureRHI, const FResolveParams& ResolveParams)
{
	...
	FD3D12Texture2DArray* SourceTexture2DArray = static_cast<FD3D12Texture2DArray*>(SourceTextureRHI->GetTexture2DArray());
	FD3D12Texture2DArray* DestTexture2DArray = static_cast<FD3D12Texture2DArray*>(DestTextureRHI->GetTexture2DArray());
	...
	else if (SourceTexture2DArray && DestTexture2DArray)
	{
		...
		if (SourceTexture2DArray != DestTexture2DArray)
		{
			...
			if (SourceTextureRHI->IsMultisampled() && !DestTextureRHI->IsMultisampled() && SourceTexture2DArray->GetSizeZ() == DestTexture2DArray->GetSizeZ())
			{
				...
				{
					CommandListHandle->ResolveSubresource(
						DestTexture2DArray->GetResource()->GetResource(),
						ArrayIndex,
						SourceTexture2DArray->GetResource()->GetResource(),
						ArrayIndex,
						Fmt
					);
				}
