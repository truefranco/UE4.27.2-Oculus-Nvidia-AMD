...
void FOculusInput::SendControllerEvents()
{
	...
	if(IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized() && FApp::HasVRFocus())
	{
		if (MessageHandler.IsValid() && GEngine->XRSystem->GetHMDDevice())
		{
			...
			if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState5(ovrpController_Remote, &OvrpControllerState)) &&
			...
			if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState5((ovrpController)(ovrpController_LTrackedRemote | ovrpController_RTrackedRemote | ovrpController_Touch), &OvrpControllerState)))
			{
			...
			if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState5((ovrpController)(ovrpController_LHand | ovrpController_RHand | ovrpController_Hands), &OvrpControllerState)))
			{
				for (FOculusControllerPair& ControllerPair : ControllerPairs)
				{
...
void FOculusInput::UpdateForceFeedback( const FOculusControllerPair& ControllerPair, const EControllerHand Hand )
{
	...
	if( ControllerState.bIsConnected && !ControllerState.bPlayingHapticEffect)
	{
		if(IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized() && FApp::HasVRFocus())
		{
			...
			if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState5((ovrpController)(ovrpController_Active | ovrpController_LTrackedRemote | ovrpController_RTrackedRemote), &OvrpControllerState)) &&
...
#endif
					ovrpControllerState5 OvrpControllerState;
					if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().GetControllerState5(ControllerTypes, &OvrpControllerState)))
					{
