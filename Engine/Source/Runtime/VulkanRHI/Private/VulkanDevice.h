...
struct FOptionalVulkanDeviceExtensions
{
	union
	{
		struct
		{
			...
			uint32 HasKHRCreateRenderpass2 : 1;
			...
			uint32 HasQcomRenderPassShaderResolve : 1;
			uint32 HasQcomFragmentDensityMapOffset : 1;
		};
		uint32 Packed;
	};
...
public:
	...
	inline const VkPhysicalDeviceFragmentDensityMapOffsetFeaturesQCOM& GetFragmentDensityMapOffsetFeatures() const
	{
		return FragmentDensityMapOffsetFeatures;
	}

	inline const VkPhysicalDeviceFragmentDensityMapOffsetPropertiesQCOM& GetFragmentDensityMapOffsetProperties() const
	{
