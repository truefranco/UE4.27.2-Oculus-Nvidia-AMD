 ...
 */
extern MODELINGCOMPONENTS_API TAutoConsoleVariable<int32> CVarModelingMaxVolumeTriangleCount;

/**
 * A tool target backed by AVolume
 */
UCLASS(Transient)
class MODELINGCOMPONENTS_API UVolumeComponentToolTarget : public UPrimitiveComponentToolTarget,
...
public:
	...
	virtual FDynamicMesh3 GetDynamicMesh() override;

	// IDynamicMeshCommitter implementation
	virtual void CommitDynamicMesh(const FDynamicMesh3& Mesh, const FDynamicMeshCommitInfo&) override;
...
UCLASS(Transient)
class MODELINGCOMPONENTS_API UVolumeComponentToolTargetFactory : public UToolTargetFactory
{
	GENERATED_BODY()
