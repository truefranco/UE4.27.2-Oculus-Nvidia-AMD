...
static TMap<FVertexDeclarationElementList, FRHIVertexDeclaration*> GVertexDeclarationCache;
static FCriticalSection GVertexDeclarationLock;

static TMap<FBlendStateInitializerRHI, FRHIBlendState*> GBlendStateCache;
static FCriticalSection GBlendStateLock;

static TMap<FRasterizerStateInitializerRHI, FRHIRasterizerState*> GRasterizerStateCache;
static FCriticalSection GRasterizerStateLock;

static TMap<FDepthStencilStateInitializerRHI, FRHIDepthStencilState*> GDepthStencilStateCache;
static FCriticalSection GDepthStencilStateLock;
...
void PipelineStateCache::Shutdown()
{
	...
	// BlendState
	{
		FScopeLock ScopeLock(&GBlendStateLock);
		for (auto Pair : GBlendStateCache)
		{
			Pair.Value->Release();
		}
		GBlendStateCache.Empty();
	}
	// RasterizerState
	{
		FScopeLock ScopeLock(&GRasterizerStateLock);
		for (auto Pair : GRasterizerStateCache)
		{
			Pair.Value->Release();
		}
		GRasterizerStateCache.Empty();
	}
	// DepthStencilState
	{
		FScopeLock ScopeLock(&GDepthStencilStateLock);
		for (auto Pair : GDepthStencilStateCache)
		{
			Pair.Value->Release();
		}
		GDepthStencilStateCache.Empty();
	}
}

FRHIVertexDeclaration*	PipelineStateCache::GetOrCreateVertexDeclaration(const FVertexDeclarationElementList& Elements)
{
	FVertexDeclarationElementList SortedElements = Elements;
	SortedElements.SortList();

	// Actual locking/contention time should be close to unmeasurable
	FScopeLock ScopeLock(&GVertexDeclarationLock);
	FRHIVertexDeclaration** Found = GVertexDeclarationCache.Find(SortedElements);
	if (Found)
	{
		return *Found;
	}

	FVertexDeclarationRHIRef NewDeclaration = RHICreateVertexDeclaration(SortedElements);

	// Add an extra reference so we don't have TRefCountPtr in the maps
	NewDeclaration->AddRef();
	GVertexDeclarationCache.Add(SortedElements, NewDeclaration);
	return NewDeclaration;
}

FRHIBlendState* PipelineStateCache::GetAndOrCreateBlendState(const FBlendStateInitializerRHI& Initializer)
{
	FScopeLock ScopeLock(&GBlendStateLock);
	FRHIBlendState** Found = GBlendStateCache.Find(Initializer);
	if (Found)
	{
		return *Found;
	}

	FBlendStateRHIRef NewBlendState = RHICreateBlendState(Initializer);

	// Add an extra reference so we don't have TRefCountPtr in the maps
	NewBlendState->AddRef();
	GBlendStateCache.Add(Initializer, NewBlendState);
	return NewBlendState;
}

FRHIRasterizerState* PipelineStateCache::GetAndOrCreateRasterizerState(const FRasterizerStateInitializerRHI& Initializer)
{
	FScopeLock ScopeLock(&GRasterizerStateLock);
	FRHIRasterizerState** Found = GRasterizerStateCache.Find(Initializer);
	if (Found)
	{
		return *Found;
	}

	FRasterizerStateRHIRef NewRasterizerState = RHICreateRasterizerState(Initializer);

	// Add an extra reference so we don't have TRefCountPtr in the maps
	NewRasterizerState->AddRef();
	GRasterizerStateCache.Add(Initializer, NewRasterizerState);
	return NewRasterizerState;
}

FRHIDepthStencilState* PipelineStateCache::GetAndOrCreateDepthStencilState(const FDepthStencilStateInitializerRHI& Initializer)
{
	FScopeLock ScopeLock(&GDepthStencilStateLock);
	FRHIDepthStencilState** Found = GDepthStencilStateCache.Find(Initializer);
	if (Found)
	{
		return *Found;
	}

	FDepthStencilStateRHIRef NewDepthStencilState = RHICreateDepthStencilState(Initializer);

	// Add an extra reference so we don't have TRefCountPtr in the maps
	NewDepthStencilState->AddRef();
	GDepthStencilStateCache.Add(Initializer, NewDepthStencilState);
