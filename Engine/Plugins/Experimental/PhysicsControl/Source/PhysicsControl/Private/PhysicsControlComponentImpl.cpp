	...
	const FPhysicsControlRecord& Record)
{
	...
	{
		return Cast<USkeletalMeshComponent>(Record.PhysicsControl.ParentMeshComponent);
	}
	return nullptr;
}
	...
	const FPhysicsControlRecord& Record)
{
	...
	{
		return Cast<USkeletalMeshComponent>(Record.PhysicsControl.ChildMeshComponent);
	}
	return nullptr;
}
	...
	const FPhysicsBodyModifier& PhysicsBodyModifier)
{
	...
	{
		return Cast<USkeletalMeshComponent>(PhysicsBodyModifier.MeshComponent);
	}
	return nullptr;
}
	...
	const FName                         InBoneName) const
{
	...
	const FReferenceSkeleton& RefSkeleton = InSkeletalMeshComponent->SkeletalMesh->GetRefSkeleton();
...
{
	for (TPair<USkeletalMeshComponent*, FCachedSkeletalMeshData>& CachedSkeletalMeshDataPair :
		CachedSkeletalMeshDatas)
	{
		...
		USkeletalMeshComponent* SkeletalMeshComponent = CachedSkeletalMeshDataPair.Key;
		if (USkeletalMeshComponent* SkeletalMesh = SkeletalMeshComponent)
		{
	...
	USkeletalMeshComponent* InSkeletalMeshComponent)
{
	...
	{
		USkeletalMeshComponent* SkeletalMeshComponent = It.Key();
...
{
	check(InSkeletalMeshComponent);
	for (TPair<USkeletalMeshComponent*, FModifiedSkeletalMeshData>& ModifiedSkeletalMeshDataPair :
		ModifiedSkeletalMeshDatas)
	{
		USkeletalMeshComponent* SkeletalMeshComponent = ModifiedSkeletalMeshDataPair.Key;
		if (SkeletalMeshComponent == InSkeletalMeshComponent)
		{
	...
	FPhysicsControlRecord& Record, FConstraintInstance* ConstraintInstance)
{
	...
	double TestAngular = (AngularSpring + AngularDamping) * FMath::Max(SMALL_NUMBER, MaxTorque);
	FVector TestLinear = (LinearSpring + LinearDamping) * FVector(
		FMath::Max(SMALL_NUMBER, MaxForce.X),
		FMath::Max(SMALL_NUMBER, MaxForce.Y),
		FMath::Max(SMALL_NUMBER, MaxForce.Z));
