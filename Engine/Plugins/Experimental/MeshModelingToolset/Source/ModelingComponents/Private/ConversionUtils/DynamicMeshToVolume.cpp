...
namespace DynamicMeshToVolumeLocals
{
	...
	bool TriangulatePolygon(FPoly& PolygonIn, TArray<FPoly>& PolygonsOut)
	{
		...
		for (const FIndex3i& Triangle : *TrianglesToUse)
		{
			...
			if (FVector::PointsAreSame(PolyToEmit.Vertices[0], PolyToEmit.Vertices[1])
				|| FVector::PointsAreSame(PolyToEmit.Vertices[0], PolyToEmit.Vertices[2])
				|| FVector::PointsAreSame(PolyToEmit.Vertices[1], PolyToEmit.Vertices[2]))
			{
			...
			PolyToEmit.Normal = (FVector)VectorUtil::Normal((FVector3f&)PolyToEmit.Vertices[0], (FVector3f&)PolyToEmit.Vertices[2], (FVector3f&)PolyToEmit.Vertices[1]);
			...
			if (!PolyToEmit.Normal.Equals(PolygonIn.Normal))
			{
				...
				PolyToEmit.TextureU = (FVector)(Rotation * (FVector3f)PolygonIn.TextureU);
				PolyToEmit.TextureV = (FVector)(Rotation * (FVector3f)PolygonIn.TextureV);
			}
			else
			{
		...
		TArray<UE::Conversion::FDynamicMeshFace>& FacesToAppendTo)
	{
		...
		for (int32 Tid : Submesh.TriangleIndicesItr())
		{
			...
			while (!VidStack.IsEmpty())
			{
				...
				FVector SubmesPre = (FVector)Submesh.GetVertex(PreviousVid);
				FVector SubmesFrom = (FVector)Submesh.GetVertex(FromVid);
				...
				FVector3d PreviousVector = Submesh.GetVertex(FromVid) - SubmesPre.GetSafeNormal();
				...
				FVector3d VectorToOther = Submesh.GetVertex(OtherVid) - SubmesFrom.GetSafeNormal();
				if (PreviousOutVector.Dot(VectorToOther) > (1.e-4f))
				{
					AdvanceToNext();
					continue;
				}
				FVector SubmesRes = (FVector)Submesh.GetVertex(ToVid);
				int32 VidAfterToVid = VidStack.IsEmpty() ? PolygonVids[0] : VidStack.Top();
				FVector3d NextVector = Submesh.GetVertex(VidAfterToVid) - SubmesRes.GetSafeNormal();
				FVector3d NextOutVector = PlaneNormal.Cross(NextVector); // points out into the disallowed half space
				VectorToOther = (FVector)Submesh.GetVertex(OtherVid) - SubmesRes.GetSafeNormal();
				if (NextOutVector.Dot(VectorToOther) > (1.e-4f))
				{
					AdvanceToNext();
					continue;
				}
...
void DynamicMeshToVolume(const FDynamicMesh3&, TArray<FDynamicMeshFace>& Faces, AVolume* TargetVolume)
{
	...
	for (FDynamicMeshFace& Face : Faces)
	{
		...
		NewPoly.Normal = (FVector)Normal;
		NewPoly.TextureU = (FVector)U;
		NewPoly.TextureV = (FVector)V;
