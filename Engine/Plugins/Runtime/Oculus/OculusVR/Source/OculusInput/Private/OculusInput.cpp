...
namespace OculusInput
{
	...
	const FKey FOculusKey::OculusTouch_Left_BatteryPercentRemaining("OculusTouch_Left_BatteryPercentRemaining");
	...
	const FKey FOculusKey::OculusTouch_Right_BatteryPercentRemaining("OculusTouch_Right_BatteryPercentRemaining");
	...
	bool FOculusInput::bPulledHapticsDesc = false;
	...
	void FOculusInput::PreInit()
	{
		...
		EKeys::AddKey(FKeyDetails(FOculusKey::OculusTouch_Left_BatteryPercentRemaining, LOCTEXT("OculusTouch_Left_BatteryPercentRemaining", "Oculus Touch (L) Battery Percentage Remaining"), FKeyDetails::GamepadKey | FKeyDetails::Axis1D | FKeyDetails::NotBlueprintBindableKey, "OculusTouch"));
		...
		EKeys::AddKey(FKeyDetails(FOculusKey::OculusTouch_Right_BatteryPercentRemaining, LOCTEXT("OculusTouch_Right_BatteryPercentRemaining", "Oculus Touch (R) Battery Percentage Remaining"), FKeyDetails::GamepadKey | FKeyDetails::Axis1D | FKeyDetails::NotBlueprintBindableKey, "OculusTouch"));
	...
	void FOculusInput::SendControllerEvents()
	{
		...
		if (IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized() && FApp::HasVRFocus())
		{
			if (MessageHandler.IsValid() && GEngine->XRSystem->GetHMDDevice())
			{
				...
				if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState6(ovrpController_Remote, &OvrpControllerState)) &&
				...
				if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState6((ovrpController)(ovrpController_LTrackedRemote | ovrpController_RTrackedRemote | ovrpController_Touch), &OvrpControllerState)))
				{
					...
					for (FOculusControllerPair& ControllerPair : ControllerPairs)
					{
						for (int32 HandIndex = 0; HandIndex < UE_ARRAY_COUNT(ControllerPair.TouchControllerStates); ++HandIndex)
						{
							...
							if (bIsCurrentlyTracked)
							{
								...
								const int OvrBatteryPercentRemaining = OvrpControllerState.BatteryPercentRemaining[HandIndex];
								...
								UE_CLOG(OVR_DEBUG_LOGGING, LogOcInput, Log, TEXT("SendControllerEvents: BatteryPercentRemaining[%d] = %f"), int(HandIndex), OvrBatteryPercentRemaining);
								...
								if (OvrBatteryPercentRemaining != State.BatteryPercentRemaining)
								{
									State.BatteryPercentRemaining = OvrBatteryPercentRemaining;
									MessageHandler->OnControllerAnalog(bIsLeft ? FOculusKey::OculusTouch_Left_BatteryPercentRemaining.GetFName() : FOculusKey::OculusTouch_Right_BatteryPercentRemaining.GetFName(), ControllerPair.UnrealControllerIndex, State.BatteryPercentRemaining);
								}

								if (OvrIndexTriggerForce != State.IndexTriggerForce)
								{
									State.IndexTriggerForce = OvrIndexTriggerForce;
									MessageHandler->OnControllerAnalog(bIsLeft ? FOculusKey::OculusTouch_Left_IndexTrigger_Force.GetFName() : FOculusKey::OculusTouch_Right_IndexTrigger_Force.GetFName(), ControllerPair.UnrealControllerIndex, State.IndexTriggerForce);
								}
				...
				if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState6((ovrpController)(ovrpController_LHand | ovrpController_RHand | ovrpController_Hands), &OvrpControllerState)))
				{
					for (FOculusControllerPair& ControllerPair : ControllerPairs)
					{
	...
	void FOculusInput::UpdateForceFeedback(const FOculusControllerPair& ControllerPair, const EControllerHand Hand)
	{
		...
		if (ControllerState.bIsConnected && !ControllerState.bPlayingHapticEffect)
		{
			if (IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized() && FApp::HasVRFocus())
			{
				...
				if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetControllerState6((ovrpController)(ovrpController_Active | ovrpController_LTrackedRemote | ovrpController_RTrackedRemote), &OvrpControllerState)) &&
					(OvrpControllerState.ConnectedControllerTypes & (ovrpController_Touch | ovrpController_LTrackedRemote | ovrpController_RTrackedRemote)))
				{
					...
					{
						SetHapticsByValue(ActualFrequency, ActualAmplitude, Hand, EOculusHandHapticsLocation::Hand);
	...
	bool FOculusInput::GetControllerOrientationAndPosition(const int32 ControllerIndex, const FName MotionSource, FRotator& OutOrientation, FVector& OutPosition, float WorldToMetersScale) const
	{
						...
						if (IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized())
						{
							...
							if (bIsPositionValid || bIsOrientationValid)
							{
								...
								if (Settings)
								{
									...
									EOculusControllerPoseAlignment ControllerPoseAlignment = Settings->ControllerPoseAlignment;
									...
									case 1:
										...
										break;
									case 2:
										ControllerPoseAlignment = EOculusControllerPoseAlignment::Grip;
										break;
									case 3:
										ControllerPoseAlignment = EOculusControllerPoseAlignment::Aim;
										break;
									default:
										break;
									}

									if (OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, CurrentFrame ? CurrentFrame->FrameNumber : OVRP_CURRENT_FRAMEINDEX, Node, &InPoseState)) && OculusHMD->ConvertPose_Internal(InPoseState.Pose, OutPose, Settings, WorldToMetersScale))
									{
										...
										{
											OutPose = OutPose * OculusHMD::FPose(FQuat(FVector(0, 1, 0), -FMath::DegreesToRadians(double(60))), FVector(-0.04, 0, -0.03) * WorldToMetersScale);
										}
										else if (FinalMotionSource == FName("LeftAim") || FinalMotionSource == FName("RightAim"))
										{
											OutPose = OutPose * OculusHMD::FPose(FQuat::Identity, FVector(0.055, 0, 0) * WorldToMetersScale);
										}
	...
	void FOculusInput::SetHapticFeedbackValues(int32 ControllerId, int32 Hand, const FHapticFeedbackValues& Values, TSharedPtr<struct FOculusHapticsDesc> HapticsDesc)
	{
		for (FOculusControllerPair& ControllerPair : ControllerPairs)
		{
			if (ControllerPair.UnrealControllerIndex == ControllerId)
				...
				if (ControllerState.bIsConnected)
				{
					if (IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().GetInitialized() && FApp::HasVRFocus())
					{
...
#endif
						ovrpControllerState6 OvrpControllerState;
						if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().GetControllerState6(ControllerTypes, &OvrpControllerState)))
						{
						...
						if (OvrpControllerState.ConnectedControllerTypes & (ovrpController_Touch | ovrpController_LTrackedRemote | ovrpController_RTrackedRemote))
						{
							...
							bool bOpenXR = (NativeXrApi == ovrpXrApi_OpenXR);
							...
							if (!bOpenXR)
							{
								if (!GetOvrpHapticsDesc((int32)Hand))
									return;
								bHapticBuffer = (bHapticBuffer && HapticBuffer->SamplingRate == OvrpHapticsDesc.SampleRateHz);
							}
							if ((OvrpControllerState.ConnectedControllerTypes & (ovrpController_Touch)) && bHapticBuffer)
							{
								...
								if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().GetControllerHapticsState2(OvrpController, &OvrpHapticsState)))
								{
								...
								float TimeToSend = 0.f;
								int WantToSend = 0;
								if (bOpenXR)
								{
									TimeToSend = GetMaxHapticDuration(EControllerHand(Hand));
									WantToSend = (int)(TimeToSend * HapticBuffer->SamplingRate);
								}
								else
								{
									...
									WantToSend = (int)ceil((float)OvrpHapticsDesc.SampleRateHz / appFrameRate) + 1;
								}
								if (WantToSend == 0)
									return;
								...
								if (OvrpHapticsState.SamplesQueued < OvrpHapticsDesc.MinimumSafeSamplesQueued + WantToSend) //trying to minimize latency
								{
									...
									else
									{
										...
										if (bOpenXR)
										{
											...
											FOculusHMDModule::GetPluginWrapper().SetControllerHapticsPcm(
												OvrpController,
												HapticsVibration);
										...
										else
										{
											...
											double EndTimePCM = FPlatformTime::Seconds();
											UE_CLOG(OVR_HAP_LOGGING, LogOcInput, Log, TEXT("SetControllerHaptics2  is finished: Total SamplesSent: %d, TimeSpent: %fms"),
												HapticBuffer->SamplesSent + SamplesSent,
												(EndTimePCM - StartTimePCM) * 1000.0);
							...
							else
							{
								...
								if (ControllerState.HapticAmplitude != Amplitude || ControllerState.HapticFrequency != Frequency)
								{
									...
									EOculusHandHapticsLocation Loc = (HapticsDesc ? HapticsDesc->Location : EOculusHandHapticsLocation::Hand);
									SetHapticsByValue(Frequency, Amplitude, (EControllerHand)Hand, Loc);
	...
	void FOculusInput::PlayHapticEffect(
		...
		bool bLoop)
	{
		if (HapticEffect)
		{
			...
			case EControllerHand::Left:
				...
				HapticsDesc_Left = MakeShareable(new FOculusHapticsDesc(Location, bAppend));
				break;
			case EControllerHand::Right:
				...
				HapticsDesc_Right = MakeShareable(new FOculusHapticsDesc(Location, bAppend));
				break;
			default:
	...
	int FOculusInput::PlayHapticEffect(EControllerHand Hand, int SamplesCount, void* Samples, int InSampleRate, bool bPCM, bool bAppend)
	{
		...
		{	//HAE
			...
			FOculusHMDModule::GetPluginWrapper().SetControllerHapticsAmplitudeEnvelope(
				OvrpController,
				HapticsVibration);
	...
	void FOculusInput::SetHapticsByValue(float Frequency, float Amplitude, EControllerHand Hand, EOculusHandHapticsLocation Location)
	{
		const ovrpController OvrpController = (Hand == EControllerHand::Left) ? ovrpController_LTouch : ovrpController_RTouch;

		ovrpXrApi NativeXrApi;
		FOculusHMDModule::GetPluginWrapper().GetNativeXrApiType(&NativeXrApi);
		//Redirect to legacy SetControllerVibration2(...) because SetControllerLocalizedVibration is only supported in OpenXR.
		if (NativeXrApi == ovrpXrApi_OpenXR)
		{
			FOculusHMDModule::GetPluginWrapper().SetControllerLocalizedVibration(OvrpController, GetOVRPHapticsLocation(Location), Frequency, Amplitude);
		...
		{
			FOculusHMDModule::GetPluginWrapper().SetControllerVibration2(OvrpController, Frequency, Amplitude);
			UE_CLOG(OVR_HAP_LOGGING, LogOcInput, Log, TEXT("SetControllerVibration2 is finished: Frequency: %f, Amplitude: %f"), Frequency, Amplitude);
		}
	}

	void FOculusInput::ProcessHaptics(const float DeltaTime)
	{
	...
	ovrpHapticsLocation FOculusInput::GetOVRPHapticsLocation(EOculusHandHapticsLocation Location)
	{
		switch (Location)
		{
		case EOculusHandHapticsLocation::Hand:
			return ovrpHapticsLocation::ovrpHapticsLocation_Hand;
		case EOculusHandHapticsLocation::Thumb:
			return ovrpHapticsLocation::ovrpHapticsLocation_Thumb;
		case EOculusHandHapticsLocation::Index:
	...
	bool FOculusInput::GetOvrpHapticsDesc(int Hand)
	{
		if (!bPulledHapticsDesc)
		{
			const ovrpController OvrpController = (EControllerHand(Hand) == EControllerHand::Left) ? ovrpController_LTouch : ovrpController_RTouch;
			// Buffered haptics is currently only supported on Touch
			if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().GetControllerHapticsDesc2(OvrpController, &OvrpHapticsDesc)))
			{
	...
	float FOculusInput::GetControllerSampleRateHz(EControllerHand Hand)
	{
		float sampleRateHz = 0.f;
		const ovrpController OvrpController = (EControllerHand(Hand) == EControllerHand::Left) ? ovrpController_LTouch : ovrpController_RTouch;
		if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().GetControllerSampleRateHz(OvrpController, &sampleRateHz)))
		{
	...
	int FOculusInput::GetMaxHapticDuration(EControllerHand Hand)
	{
		const ovrpController OvrpController = (EControllerHand(Hand) == EControllerHand::Left) ? ovrpController_LTouch : ovrpController_RTouch;
		if (!GetOvrpHapticsDesc((int32)Hand))
			return 0;
