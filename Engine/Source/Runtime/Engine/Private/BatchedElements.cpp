			...
			&& (Filter & EBlendModeFilter::OpaqueAndMasked))
		{
			...
			if ( ThickLines.Num() > 0 )
			{
				...
				while (LineIndex < ThickLines.Num())
				{
					...
					GraphicsPSOInit.RasterizerState = PipelineStateCache::GetAndOrCreateRasterizerState(Initializer);
			...
			if (WireTris.Num() > 0)
			{
				...
				*/
				...
				while( MinTri < TotalTris )
				{
					...
					// PSOCache Note: change RasterizerState direct creation to use managed creation 
					GraphicsPSOInit.RasterizerState = PipelineStateCache::GetAndOrCreateRasterizerState(Initializer);
					PrepareShaders(RHICmdList, GraphicsPSOInit, FeatureLevel, SE_BLEND_Opaque, Transform, bNeedToSwitchVerticalAxis, BatchedElementParameters, GWhiteTexture, bHitTesting, Gamma, NULL, &View);
					RHICmdList.SetStencilRef(StencilRef);
