from ModuleConverterInterface import ModuleConverterInterface
import unreal as ue
from unreal import FXConverterUtilitiesLibrary as ue_fx_utils
import Paths


class CascadeConstantAccelerationConverter(ModuleConverterInterface):

    @classmethod
    def get_input_cascade_module(cls):
        return ue.ParticleModuleAccelerationConstant

    @classmethod
    def convert(cls, args):
        cascade_module = args.get_cascade_module()
        niagara_emitter_context = args.get_niagara_emitter_context()
        
        # find/add the module script for acceleration force
        acceleration_force_script_asset_data = ue.AssetData(Paths.script_acceleration_force)
        acceleration_force_script = niagara_emitter_context.find_or_add_module_script(
            "ConstantAccelerationForce",
            acceleration_force_script_asset_data,
            ue.ScriptExecutionCategory.PARTICLE_UPDATE)

        # get all properties from the cascade constant acceleration module
        acceleration_vec = ue_fx_utils.get_particle_module_constant_acceleration_props(cascade_module)

        # make an input to apply the acceleration vector
        acceleration_input = ue_fx_utils.create_script_input_vector(acceleration_vec)

        # set the acceleration vector value
        acceleration_force_script.set_parameter("Acceleration", acceleration_input)

