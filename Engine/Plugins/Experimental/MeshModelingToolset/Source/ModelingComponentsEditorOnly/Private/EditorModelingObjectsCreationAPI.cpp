...
FCreateMeshObjectResult UEditorModelingObjectsCreationAPI::CreateVolume(FCreateMeshObjectParams&& CreateMeshParams)
{
	...
	if (CreateMeshParams.TypeHintClass
		&& Cast<AVolume>(CreateMeshParams.TypeHintClass->GetDefaultObject(false)) != nullptr)
	{
...
FCreateMeshObjectResult UEditorModelingObjectsCreationAPI::CreateDynamicMeshActor(FCreateMeshObjectParams&& CreateMeshParams)
{
	...
	USimpleDynamicMeshComponent* NewComponent = NewActor->GetDynamicMeshComponent();

	// assume that DynamicMeshComponent always has tangents on it's internal UDynamicMesh
	NewComponent->SetTangentsType(EDynamicMeshTangentCalcType::ExternallyCalculated);
...
{
	FMaterialAssetOptions AssetOptions;
	...
	FMaterialAssetResults ResultData;
	ECreateMaterialResult AssetResult = CreateDuplicateMaterial(
		CreateMaterialParams.MaterialToDuplicate,
		AssetOptions,
		ResultData);

	if (AssetResult != ECreateMaterialResult::Ok)
	{
