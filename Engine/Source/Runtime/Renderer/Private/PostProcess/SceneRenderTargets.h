...
public:
	...
	*/
	...
	// AppSpaceWarp
	const FTexture2DRHIRef& GetMotionVectorTexture() const { return (const FTexture2DRHIRef&)HmdMotionVectorTexture->GetRenderTargetItem().ShaderResourceTexture; }
	const FTexture2DRHIRef& GetMotionVectorDepthTexture() const { return (const FTexture2DRHIRef&)HmdMotionVectorDepthTexture->GetRenderTargetItem().ShaderResourceTexture; }
	...
	// AppSpaceWarp
	bool IsHmdMotionVectorTextureAllocated() const { return bAllocatedHmdMotionVectorTexture; }
...
private:
	...
	void AllocateHmdMotionVectorTexture(FRHICommandList& RHICmdList);
	...
	// AppSpaceWarp
	/** True if the HMD motion vector texture */
	bool bAllocatedHmdMotionVectorTexture;
	/** True when HMD motion vector RTs need be reallocated from the RT pool */
	bool bNeedRettachHmdMotionVectorTexture;
