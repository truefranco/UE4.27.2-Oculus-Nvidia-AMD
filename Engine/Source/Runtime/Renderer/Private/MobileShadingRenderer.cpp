			...
			// Finish rendering for each view, or the full stereo buffer if enabled
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
				{
					...
					if (View.ShouldRenderView()) {
						...
						AddMobilePostProcessingPasses(GraphBuilder, View, PostProcessingInputs, NumMSAASamples > 1);
					}
				}
			}
		}
...
// AppSpaceWarp
void FMobileSceneRenderer::RenderForwardHmdMotionVectors(FRHICommandListImmediate& RHICmdList, FRHITexture* VelocityTexture, FRHITexture* VelocityDepthTexture, const TArrayView<const FViewInfo*> ViewList)
{
	SCOPED_NAMED_EVENT(RenderForwardHmdMotionVectors, FColor::Magenta);

	if (VelocityTexture != nullptr && VelocityDepthTexture != nullptr)
	{
		const FViewInfo& View = *ViewList[0];
		RHICmdList.Transition(FRHITransitionInfo(VelocityTexture, ERHIAccess::Unknown, ERHIAccess::RTV));
		RHICmdList.Transition(FRHITransitionInfo(VelocityDepthTexture, ERHIAccess::Unknown, ERHIAccess::DSVWrite));

		FRHIRenderPassInfo RPInfo(VelocityTexture, ERenderTargetActions::Clear_Store);
		...
		RPInfo.DepthStencilRenderTarget.DepthStencilTarget = VelocityDepthTexture;
		...
		RHICmdList.Transition(FRHITransitionInfo(VelocityTexture, ERHIAccess::RTV, ERHIAccess::SRVMask));
		RHICmdList.Transition(FRHITransitionInfo(VelocityDepthTexture, ERHIAccess::DSVWrite, ERHIAccess::DSVRead));
		RHICmdList.ImmediateFlush(EImmediateFlushType::DispatchToRHIThread);
	}
}
...
FRHITexture* FMobileSceneRenderer::RenderForward(FRHICommandListImmediate& RHICmdList, const TArrayView<const FViewInfo*> ViewList)
{
	...
	ERenderTargetActions ColorTargetsAction[2] = { ERenderTargetActions::Clear_Store, GBufferAction };
	FRHITexture* ColorTargets[2] = { nullptr, nullptr, };
	...
	if (ShouldRenderHmdMotionVectors(SceneContext, HmdMotionVectorTexture, HmdMotionVectorDepthTexture) && !View.bIsSceneCapture && !View.bIsReflectionCapture)
	{
		RenderForwardHmdMotionVectors(RHICmdList, HmdMotionVectorTexture, HmdMotionVectorDepthTexture, ViewList);
	}

	const bool bUseColorGradingLUT = (bUseMobileTonemapSubpass && (View.Family->EngineShowFlags.ColorGrading != 0));
	...
	if (bGammaSpace && !bRenderToSceneColor)
	{
		if (bMobileMSAA || bUseMobileTonemapSubpass)
		{
			...
			{
				ColorTargetsAction[0] = ERenderTargetActions::Clear_DontStore;
				RHICmdList.Transition(FRHITransitionInfo(SceneColorResolve, ERHIAccess::Unknown, ERHIAccess::RTV));
			}
			else
			{
	...
	else
	{
		...
		else
		{
			...
			ColorTargetsAction[0] = ERenderTargetActions::Clear_Store;
		}
	...
	if (GIsEditor && bUseMobileTonemapSubpass)
	{
		...
		ColorTargets[1] = SceneContext.GetGBufferATexture().GetReference();

		ColorTargets[0] = SceneColor;
		ColorTargetsAction[0] = ERenderTargetActions::Clear_Store;

		int32 ColorTargetIndex = 0;
		for (; ColorTargetIndex < UE_ARRAY_COUNT(ColorTargets); ++ColorTargetIndex)
		{
			if (ColorTargets[ColorTargetIndex] != nullptr)
			{
				SceneColorRenderPassInfo.ColorRenderTargets[ColorTargetIndex].RenderTarget = ColorTargets[ColorTargetIndex];
				SceneColorRenderPassInfo.ColorRenderTargets[ColorTargetIndex].ResolveTarget = nullptr;
				SceneColorRenderPassInfo.ColorRenderTargets[ColorTargetIndex].ArraySlice = -1;
				SceneColorRenderPassInfo.ColorRenderTargets[ColorTargetIndex].MipIndex = 0;
				SceneColorRenderPassInfo.ColorRenderTargets[ColorTargetIndex].Action = ColorTargetsAction[ColorTargetIndex];
			}
		}
	}

	SceneColorRenderPassInfo.SubpassHint = bUseDepthSubpass ? ESubpassHint::DepthReadSubpass : ESubpassHint::None;
	SceneColorRenderPassInfo.SubpassHint |= (bUseMobileTonemapSubpass ? ESubpassHint::MobileTonemapSubpass : ESubpassHint::None);
	if (!bIsFullPrepassEnabled)
	{
	...
	if (bUseMobileTonemapSubpass)
	{
		MobileTonemapSubpass(RHICmdList, bUseColorGradingLUT, View);
	}
...
void FMobileSceneRenderer::PostRenderBasePass(FRHICommandListImmediate& RHICmdList)
{
...
bool FMobileSceneRenderer::RequiresMultiPass(FRHICommandListImmediate& RHICmdList, const FViewInfo& View) const
{
	...
	if (IsMobileDeferredShadingEnabled(ShaderPlatform) || (GIsEditor && IsMobileTonemapSubpassEnabled(ShaderPlatform)))
	{
		// TODO: add GL support
		return true;
	}
...
void FMobileSceneRenderer::MobileTonemapSubpass(FRHICommandListImmediate& RHICmdList, bool bUseColorGradingLUT, const FViewInfo& View)
{
	RHICmdList.NextSubpass();
	FRDGBuilder GraphBuilder(RHICmdList);

	FMobilePostProcessingSubpassInputs PostProcessingSubpassInputs;
	...
	if (bUseColorGradingLUT)
	{
		...
		{
			ColorGradingTexture = View.GetTonemappingLUT()->GetShaderResourceRHI();
		}
		else
		{
			const FViewInfo* PrimaryView = static_cast<const FViewInfo*>(View.Family->Views[0]);
			ColorGradingTexture = PrimaryView->GetTonemappingLUT()->GetShaderResourceRHI();
		}

		PostProcessingSubpassInputs.ColorGradingTexture = ColorGradingTexture;
	}

	AddMobilePostProcessingSubpass(GraphBuilder, View, PostProcessingSubpassInputs);
	GraphBuilder.Execute();
}
...
{
	return !(IsUsingMobilePixelProjectedReflection(ShaderPlatform) || IsUsingMobileAmbientOcclusion(ShaderPlatform) || ShouldRenderVelocities() || bIsFullPrepassEnabled || bDeferredShading || (GIsEditor && bTonemapSubpass));
}
