...
const FString FAndroidDeviceProfileSelectorModule::GetDeviceProfileName(const TMap<FString, FString>& DeviceParameters)
{
	...
	FString HMDSystemName = DeviceParameters.FindChecked("HMDSystemName");
	...
	UE_LOG(LogAndroidDPSelector, Log, TEXT("  HMDSystemName: %s"), *HMDSystemName);

	ProfileName = FAndroidDeviceProfileSelector::FindMatchingProfile(GPUFamily, GLVersion, AndroidVersion, DeviceMake, DeviceModel, DeviceBuildNumber, VulkanAvailable, VulkanVersion, UsingHoudini, Hardware, Chipset, HMDSystemName, ProfileName);
