...
public:
	...
	inline const VkAttachmentDescription2* GetAttachmentDescriptions() const { return Desc; }
	...
	inline const VkAttachmentReference2* GetColorAttachmentReferences() const { return NumColorAttachments > 0 ? ColorReferences : nullptr; }
	inline const VkAttachmentReference2* GetResolveAttachmentReferences() const { return bHasResolveAttachments ? ResolveReferences : nullptr; }
	inline const VkAttachmentReference2* GetDepthStencilAttachmentReference() const { return bHasDepthStencil ? &DepthStencilReference : nullptr; }
...
protected:
	...
	VkAttachmentReference2 ColorReferences[MaxSimultaneousRenderTargets];
	VkAttachmentReference2 DepthStencilReference;
	VkAttachmentReference FragmentDensityReference;
	VkAttachmentReference2 ResolveReferences[MaxSimultaneousRenderTargets];
	VkAttachmentReference2 InputAttachments[MaxSimultaneousRenderTargets + 1];

	// Depth goes in the "+1" slot and the Shading Rate texture goes in the "+2" slot.
	VkAttachmentDescription2 Desc[MaxSimultaneousRenderTargets * 2 + 2];
