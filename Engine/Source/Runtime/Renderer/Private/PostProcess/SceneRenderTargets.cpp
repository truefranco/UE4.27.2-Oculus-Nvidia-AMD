...
FSceneRenderTargets::FSceneRenderTargets(const FViewInfo& View, const FSceneRenderTargets& SnapshotSource)
	...
	// AppSpaceWarp
	, HmdMotionVectorTexture(GRenderTargetPool.MakeSnapshot(SnapshotSource.HmdMotionVectorTexture))
	, HmdMotionVectorDepthTexture(GRenderTargetPool.MakeSnapshot(SnapshotSource.HmdMotionVectorDepthTexture))
...
void FSceneRenderTargets::AllocSceneColor(FRHICommandList& RHICmdList)
{
	...
	{
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(BufferSize, SceneColorBufferFormat, DefaultColorClear, TexCreate_None, TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, false));
...
// AppSpaceWarp
void FSceneRenderTargets::AllocateHmdMotionVectorTexture(FRHICommandList& RHICmdList)
{
	ensureMsgf(bRequireMultiView, TEXT("Only enable under mobile multiview"));
	IStereoRenderTargetManager* const StereoRenderTargetManager = GEngine->StereoRenderingDevice->GetRenderTargetManager();
	HmdMotionVectorTexture.SafeRelease();
	HmdMotionVectorDepthTexture.SafeRelease();

	FTexture2DRHIRef MVTexture;
	FIntPoint MVTextureSize;
	FTexture2DRHIRef MVDepthTexture;
	FIntPoint MVDepthTextureSize;
	EPixelFormat MvPixelFormat = PF_FloatRGBA;
	bAllocatedHmdMotionVectorTexture = StereoRenderTargetManager && StereoRenderTargetManager->AllocateMotionVectorTexture(0, MvPixelFormat, 0, TexCreate_None, TexCreate_None, MVTexture, MVTextureSize, MVDepthTexture, MVDepthTextureSize);
	if (bAllocatedHmdMotionVectorTexture)
	{
		// Only compatible with multiview.
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(MVTextureSize, MvPixelFormat, FClearValueBinding::Black, TexCreate_None, TexCreate_RenderTargetable | TexCreate_ShaderResource, false));
		Desc.ArraySize = 2;
		Desc.bIsArray = true;
		Desc.NumSamples = 1;

		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, HmdMotionVectorTexture, TEXT("MotionVector"));
		const uint32 OldElementSize = HmdMotionVectorTexture->ComputeMemorySize();
		HmdMotionVectorTexture->GetRenderTargetItem().ShaderResourceTexture = HmdMotionVectorTexture->GetRenderTargetItem().TargetableTexture = MVTexture;
		GRenderTargetPool.UpdateElementSize(HmdMotionVectorTexture, OldElementSize);

		FPooledRenderTargetDesc DepthDesc(FPooledRenderTargetDesc::Create2DDesc(MVTextureSize, PF_DepthStencil, FClearValueBinding::DepthFar, TexCreate_None, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource, false));
		DepthDesc.ArraySize = 2;
		DepthDesc.bIsArray = true;
		DepthDesc.NumSamples = 1;

		GRenderTargetPool.FindFreeElement(RHICmdList, DepthDesc, HmdMotionVectorDepthTexture, TEXT("MotionVectorDepth"));
		const uint32 OldElementSizeDepth = HmdMotionVectorDepthTexture->ComputeMemorySize();
		if (MVDepthTexture != NULL)
		{
			HmdMotionVectorDepthTexture->GetRenderTargetItem().ShaderResourceTexture = HmdMotionVectorDepthTexture->GetRenderTargetItem().TargetableTexture = MVDepthTexture;
		}
		GRenderTargetPool.UpdateElementSize(HmdMotionVectorDepthTexture, OldElementSizeDepth);
	}
}
...
void FSceneRenderTargets::AllocateRenderTargets(FRHICommandListImmediate& RHICmdList, const int32 NumViews)
{
	...
	const bool bStereo = GEngine->StereoRenderingDevice.IsValid() && GEngine->StereoRenderingDevice->IsStereoEnabled();
	IStereoRenderTargetManager* const StereoRenderTargetManager = bStereo ? GEngine->StereoRenderingDevice->GetRenderTargetManager() : nullptr;
	if (bRequireMultiView && StereoRenderTargetManager && (StereoRenderTargetManager->NeedReAllocateMotionVectorTexture(HmdMotionVectorTexture, HmdMotionVectorDepthTexture) || bNeedRettachHmdMotionVectorTexture))
	{
		AllocateHmdMotionVectorTexture(RHICmdList);
		bNeedRettachHmdMotionVectorTexture = false;
	}
}
...
void FSceneRenderTargets::ReleaseAllTargets()
{
	...
	HmdMotionVectorTexture.SafeRelease();
	HmdMotionVectorDepthTexture.SafeRelease();
	bAllocatedHmdMotionVectorTexture = false;
	bNeedRettachHmdMotionVectorTexture = true;

	LastStereoSize = FIntPoint(0, 0);
}
	...
	FMobileSceneTextureUniformParameters& SceneTextureParameters)
{
	...
	// Mobile GBuffer
	{
		const bool bCanReadGBufferUniforms = IsMobileDeferredShadingEnabled(GMaxRHIShaderPlatform) || (GIsEditor && IsMobileTonemapSubpassEnabled(GMaxRHIShaderPlatform));
