...
UMRUKBlobShadowComponent::UMRUKBlobShadowComponent()
{
	...
	const ConstructorHelpers::FObjectFinder<UMaterialInstance> BlobShadowMaterialAsset(TEXT("/MRUtilityKit/Materials/MI_BlobShadow"));
	...
	{
		UE_LOG(LogMRUK, Log, TEXT("Blob shadow couldn't find blob shadow material in /MRUtilityKit/Materials/MI_BlobShadow"));
	}
...
void UMRUKBlobShadowComponent::UpdatePlaneSizeAndPosition()
{
	...
	const bool bHasHit = UKismetSystemLibrary::SphereTraceSingle(this, Origin, Origin + FVector::DownVector * MaxVerticalDistance, Extent.Size() * 0.5f, TraceTypeQuery1,
