...
// AppSpaceWarp 
bool FOpaqueVelocityMeshProcessor::PrimitiveCanHaveVelocity(EShaderPlatform ShaderPlatform, bool allowStatic, const FPrimitiveSceneProxy* PrimitiveSceneProxy)
{
	...
	if (!allowStatic && !PrimitiveSceneProxy->IsMovable())
	{
		return false;
	}
...
// AppSpaceWarp
bool FOpaqueVelocityMeshProcessor::PrimitiveHasVelocityForFrame(bool allowStatic, const FPrimitiveSceneProxy* PrimitiveSceneProxy)
{
	if (!allowStatic && !PrimitiveSceneProxy->AlwaysHasVelocity())
	{
	...
	int32 StaticMeshId)
{
	...
	bool allowStatic = (ViewInfo == nullptr) ? FVelocityRendering::IsVelocityWithFullDepthSupported() : ViewInfo->bIncludeStaticInVelocityPass;
	if (!PrimitiveCanHaveVelocity(ShaderPlatform, allowStatic, PrimitiveSceneProxy))
	{
		return;
	}
