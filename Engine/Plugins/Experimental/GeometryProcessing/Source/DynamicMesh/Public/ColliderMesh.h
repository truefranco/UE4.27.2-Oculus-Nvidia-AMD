...
public:
	DYNAMICMESH_API FColliderMesh();
	DYNAMICMESH_API FColliderMesh(const FDynamicMesh3& SourceMesh, const FBuildOptions& BuildOptions = FBuildOptions());

	DYNAMICMESH_API void Initialize(const FDynamicMesh3& SourceMesh, const FBuildOptions& BuildOptions = FBuildOptions());

	DYNAMICMESH_API void Reset(bool bReleaseMemory);
	...
	uint64 GetShapeTimestamp() const { return 0; }
	...
	DYNAMICMESH_API int32 GetSourceVertexID(int32 VertexID) const;
	...
	DYNAMICMESH_API int32 GetSourceTriangleID(int32 TriangleID) const;
	 ...
	 */
	DYNAMICMESH_API bool FindNearestHitTriangle( const FRay3d& Ray, double& RayParameterOut, int& HitTriangleIDOut) const;
	 ...
	 */
	DYNAMICMESH_API int FindNearestTriangle( const FVector3d& Point, double& NearestDistSqrOut, const IMeshSpatial::FQueryOptions& Options = IMeshSpatial::FQueryOptions() ) const;
	 ...
	 */
	DYNAMICMESH_API TMeshAABBTree3<FColliderMesh>* GetRawAABBTreeUnsafe();
...
public:
	...
	DYNAMICMESH_API virtual FVector3d Project(const FVector3d& Point, int Identifier = -1) override;
	DYNAMICMESH_API virtual FVector3d Project(const FVector3d& Point, FVector3d& ProjectNormalOut, int Identifier = -1) override;
};
