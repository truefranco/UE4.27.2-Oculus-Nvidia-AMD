...
{
	DyMesh = MakeUnique<FDynamicMesh3>(EMeshComponents::FaceGroups);
	InitializeMesh();
}
...
UDynamicMesh* UDynamicMesh::Reset()
{
	...
	{
		check(&EditMesh == DyMesh.Get());		// assuming that EditMesh is internal mesh here...
		InitializeMesh();
	}, ChangeInfo);
	return this;
}
...
{
	DyMesh->Clear();
	DyMesh->EnableTriangleGroups();
	DyMesh->EnableAttributes();
	DyMesh->Attributes()->EnableMaterialID();

	if (MeshGenerator != nullptr && bEnableMeshGenerator)
	{
		MeshGenerator->Generate(*DyMesh);
	}
}
...
{
	return DyMesh->TriangleCount() == 0;
}

int32 UDynamicMesh::GetTriangleCount() const
{
	return DyMesh->TriangleCount();
}


void UDynamicMesh::SetMesh(const FDynamicMesh3& MoveMesh)
{
	FDynamicMeshChangeInfo ChangeInfo;
	ChangeInfo.Type = EDynamicMeshChangeType::GeneralEdit;
...
void UDynamicMesh::SetMesh(FDynamicMesh3&& MoveMesh)
{
	FDynamicMeshChangeInfo ChangeInfo;
	ChangeInfo.Type = EDynamicMeshChangeType::GeneralEdit;
	EditMeshInternal([&](FDynamicMesh3& EditMesh) {
...
void UDynamicMesh::ProcessMesh(TFunctionRef<void(const FDynamicMesh3&)> ProcessFunc) const
{
...
void UDynamicMesh::EditMeshInternal(TFunctionRef<void(FDynamicMesh3&)> EditFunc, const FDynamicMeshChangeInfo& ChangeInfo, bool bDeferChangeEvents)
{
	...
	if (DyMesh->HasTriangleGroups() == false)
	{
		DyMesh->EnableTriangleGroups();
	}
	if (DyMesh->HasAttributes() == false)
	{
		DyMesh->EnableAttributes();
	}
	if (DyMesh->Attributes()->HasMaterialID() == false)
	{
		DyMesh->Attributes()->EnableMaterialID();
	}
...
{
	TUniquePtr<FDynamicMesh3> ReturnMesh = MoveTemp(DyMesh);
	InitializeNewMesh();
	return ReturnMesh;
}
...
void UDynamicMesh::SetMeshGenerator(UDynamicMeshGenerator* NewGenerator)
{
...
{
	// ignore empty meshes
	if (DyMesh->TriangleCount() == 0 && DyMesh->VertexCount() == 0)
	{
		return;
	}
	...
	if (DyMesh->TriangleCount() < CVarDynamicMeshTextBasedDupeTriangleCountThreshold.GetValueOnGameThread())
	{
		// serialize our mesh
...
void UDynamicMesh::ImportCustomProperties(const TCHAR* SourceText, FFeedbackContext* Warn)
{
	...
	if (FParse::Command(&SourceText, TEXT("MeshData")))	
	{
		...
		UE_LOG(LogDynamicMesh, Warning, TEXT("UDynamicMesh text-based property serialization incomplete, generating box as placeholder. Try increasing geometry.DynamicMesh.TextBasedDupeTriThreshold, or geometry.DynamicMesh.DupeStashTimeout."))
		FMinimalBoxMeshGenerator BoxGen;
		BoxGen.Box = FOrientedBox3d(FVector3d::Zero(), 50.0 * FVector3d::One());
...
UDynamicMesh* UDynamicMeshPool::RequestMesh()
{
	...
	{
		UE_LOG(LogDynamicMesh, Warning, TEXT("UDynamicMeshPool Threshold of %d Allocated Meshes exceeded! Releasing references to all current meshes and forcing a garbage collection."), CVarDynamicMeshPoolMaxPoolSizeThreshold.GetValueOnGameThread());
