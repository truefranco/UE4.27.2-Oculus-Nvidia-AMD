...
class TransformAttributes(AttributesContainerBase):
	def __init__(self):
		...
		attribute.construct_usd_attribute = lambda usd_node, up_axis : usd_node.AddTranslateOp()
		attribute.get_from_actor = lambda x : x.get_actor_location().to_tuple()
		attribute.convert_unreal_to_usd = lambda x, unit_scale, up_axis : TransformAttributes.convert_location_from_unreal(x, unit_scale, up_axis)
		...
		attribute.construct_usd_attribute = lambda usd_node, up_axis : usd_node.AddRotateXYZOp() if up_axis == 'z' else usd_node.AddRotateXZYOp()
		attribute.get_from_actor = lambda x : x.get_actor_rotation().to_tuple()
		attribute.convert_unreal_to_usd = lambda x, unit_scale, up_axis : TransformAttributes.convert_rotation_from_unreal(x, unit_scale, up_axis)

		attribute = self.define_attribute("Scale", 3)
		self.scale = attribute
		attribute.sequencer_channel_names = ["Scale.X", "Scale.Y", "Scale.Z"]
		attribute.construct_usd_attribute = lambda usd_node, up_axis : usd_node.AddScaleOp()
		attribute.get_from_actor = lambda x : x.get_actor_scale3d().to_tuple()
		attribute.convert_unreal_to_usd = lambda x, unit_scale, up_axis : TransformAttributes.convert_scale_from_unreal(x, unit_scale, up_axis)

		self.usd_camera_import_rotation = unreal.Quat()
		self.usd_camera_import_rotation.set_from_euler([0, 0, -90]) # roll, pitch, yaw

	@staticmethod
	def convert_location_from_unreal(v, unit_scale, up_axis='z'):
		if up_axis == 'z':
			return Gf.Vec3d(v[0] * unit_scale, -v[1] * unit_scale, v[2] * unit_scale)
		else: 
			return Gf.Vec3d(v[0] * unit_scale, v[2] * unit_scale, v[1] * unit_scale)

	@staticmethod
	def convert_rotation_from_unreal(v, unit_scale=1, up_axis='z'):
		if up_axis == 'z':
	...
	@staticmethod
	def convert_scale_from_unreal(v, unit_scale=1, up_axis='z'):
		if up_axis == 'z':
			return Gf.Vec3f(v[0], v[1], v[2])
		else: 
			return Gf.Vec3f(v[0], v[2], v[1])
	...
	def usd_transform_from_unreal_transform(self, unreal_transform, unit_scale=1.0, up_axis='y'):
		location = TransformAttributes.convert_location_from_unreal(unreal_transform.translation.to_tuple(), unit_scale, up_axis)
		rotation = TransformAttributes.convert_rotation_from_unreal(unreal_transform.rotation.rotator().to_tuple(), unit_scale, up_axis)
		scale = TransformAttributes.convert_scale_from_unreal(unreal_transform.scale3d.to_tuple(), unit_scale, up_axis)
