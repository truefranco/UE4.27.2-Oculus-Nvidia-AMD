...
public:
	...
	static bool CachePSO(const FPipelineCacheFileFormatPSO & NewEntry, std::function<void(uint32)> NewPSODelegate);
	static FPipelineStateStats* RegisterPSOStats(const FPipelineCacheFileFormatPSO & PSO);
...
private:
	...
	static TMap<FPipelineCacheFileFormatPSO, FPSOUsageData> PSOToUsage;		// Fast check structure - Not saved (External state cache runtime hash to seen usage data)
