...
if usd_unreal.developer_mode == True:
	...
	importlib.reload(usd_unreal.diff_usd_files)
	importlib.reload(usd_unreal.automated_tests)
	importlib.reload(usd_unreal.capture_sequence)

	#importlib.reload(usd_unreal.capture_sequence_class)
	#importlib.reload(usd_unreal.export_sequence_class)
	#importlib.reload(usd_unreal.import_level_class)
	#importlib.reload(usd_unreal.export_level_class)
...
class USDCreateFile:
	...
	def run(self):
		...
		if getattr(self, 'sub_layer_paths', None) != None:
			for i in range(len(self.sub_layer_paths)):
