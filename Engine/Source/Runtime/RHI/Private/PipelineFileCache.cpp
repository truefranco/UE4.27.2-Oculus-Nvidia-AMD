...
/*static*/ bool FPipelineCacheFileFormatPSO::Init(FPipelineCacheFileFormatPSO& PSO, FGraphicsPipelineStateInitializer const& Init)
{
	...
	check (Init.BoundShaderState.VertexDeclarationRHI->IsValid());
	{
		...
		PSO.GraphicsDesc.VertexDescriptor.SortList();
	}
		...
		if(Type == Other.Type)
		{
			switch(Type)
			{
				...
				case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
				{
					...
					GraphicsDesc.MultiViewCount == Other.GraphicsDesc.MultiViewCount && 
					GraphicsDesc.HasFragmentDensityAttachment == Other.GraphicsDesc.HasFragmentDensityAttachment &&
...
//-----------------------------------------------------------------------------------------------------------------------
bool FPipelineFileCache::CachePSO(const FPipelineCacheFileFormatPSO& NewEntry, std::function<void(uint32)> NewPSODelegate )
{
	if(IsPipelineFileCacheEnabled() && (LogPSOtoFileCache() || ReportNewPSOs()))
	{
		...
		{
			//-------------------------------------------------------------------------------------------------
			// PSOCache note: using RunTimeHash as key for table lookup doesn't offer protection against
			// hash collision. Also, hash collision is happening because RunTimeHash does not include all
			// attributes as part of the hash calculation. Since this code path is only invoked during
			// PSO recording, I am using the slower FPipelineCacheFileFormatPSO for key so that hashing
			// can be more robust.
			//-------------------------------------------------------------------------------------------------
			...
			if(PSOUsage == nullptr || !IsReferenceMaskSet(FPipelineFileCache::GameUsageMask, PSOUsage->UsageMask))
			{
				...
				{	
					...
					if (!FileCache->IsPSOEntryCached(NewEntry, &CurrentUsageData))
					{
						...
						if (bActuallyNewPSO && NewPSODelegate)
						{
							NewPSODelegate(PSOHash);
						}
					}
					...
					// return true to indicate new entry has been added
					return true;
				}
	...
	// return false to indicate no new entry added
	return false;
}
...
FPipelineStateStats* FPipelineFileCache::RegisterPSOStats(const FPipelineCacheFileFormatPSO & PSO)
{
	...
	if(IsPipelineFileCacheEnabled() && LogPSOtoFileCache())
	{
		...
		if(FileCache)
		{
			uint32 PSOHash = GetTypeHash(PSO);
...
FPipelineStateStats* FPipelineFileCache::CacheGraphicsPSO(FGraphicsPipelineStateInitializer const& Initializer)
{
	if (IsPipelineFileCacheEnabled() && (LogPSOtoFileCache() || ReportNewPSOs()))
	{
		...
		FPipelineFileCache::CachePSO(NewEntry, [NewEntry](uint32 PSOHash){
...
void FPipelineFileCache::CacheRayTracingPSO(const FRayTracingPipelineStateInitializer& Initializer)
{
	...
	for (TArrayView<FRHIRayTracingShader*>& Table : ShaderTables)
	{
		...
		{
			FPipelineCacheFileFormatPSO NewEntry;
			bool bOK = FPipelineCacheFileFormatPSO::Init(NewEntry, FPipelineFileCacheRayTracingDesc(Initializer, Shader));
			check(bOK);

			bool newEntryAdded = FPipelineFileCache::CachePSO(NewEntry, [&NewEntry](uint32 PSOHash){
