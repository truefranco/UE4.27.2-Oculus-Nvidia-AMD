...
//				AppType = ".Client";		// should always be empty now; fix up the name in batch file instead
		...
		public void WriteVSCodeDebugProject(string ProjectName, string PackageName, string SymbolsDir, string LLDBVisPath)
		{
			DirectoryReference UE4ProjectRoot = UnrealBuildTool.RootDirectory;
			DirectoryReference VSCodeDir = DirectoryReference.Combine(UE4ProjectRoot, ".vscode");
			DirectoryReference.CreateDirectory(VSCodeDir);

			Log.TraceInformation("\nWriting VSCode android debug project in " + VSCodeDir.FullName);

			JsonFile OutFile = new JsonFile();
			string[] ConfigTypes = new string[] { "launch", "attach" };
			...
			OutFile.BeginRootObject();
			{
				...
				foreach (string ConfigType in ConfigTypes)
				{
					OutFile.BeginObject();
					{
						...
						OutFile.AddField("name", ProjectName + " : " + ConfigType);
						OutFile.AddField("request", ConfigType);
						...
						OutFile.BeginObject("android");
						{
							...
							OutFile.BeginObject("lldbConfig");
							{
								...
								OutFile.AddUnnamedField(SymbolsDir.Replace("\\", "\\\\"));
								OutFile.EndArray();
								...
								OutFile.AddUnnamedField("command script import " + LLDBVisPath.Replace("\\", "\\\\"));
	...
	//				default: throw new BuildException("Unknown NDK architecture '{0}'", NDKArch);
		...
		private static UnrealTargetPlatform RuntimePlatform = GetRuntimePlatform();
		private static string GetPlatformNDKHostName()
		{
			if (RuntimePlatform == UnrealTargetPlatform.Linux)
			{
				return "linux-x86_64";
			}
		...
		void CopyVulkanValidationLayers(string UE4BuildPath, string UE4Arch, string NDKArch, string Configuration)
		{
			...
			bool packageOculus = IsPackagingForOculusMobile(Ini);

			bool bCopyVulkanLayers = (bSupportsVulkan || bSupportsVulkanSM5) && (Configuration == "Debug" || Configuration == "Development") && !packageOculus;
		...
		private string GetAllBuildSettings(AndroidToolChain ToolChain, bool bForDistribution, bool bMakeSeparateApks, bool bPackageDataInsideApk, bool bDisableVerifyOBBOnStartUp, bool bUseExternalFilesDir, string TemplatesHashCode)
		{
			...
			CurrentSettings.Append(GetOculusBuildSettings());
		...
		private string GetOculusSplashColorSpace()
		{
			bool bPackageForOculus = IsPackagingForOculusMobile();
			if (bPackageForOculus)
			{
				...
				string OculusSplashColorSpace;
				Ini.GetString("/Script/OculusHMD.OculusHMDRuntimeSettings", "ColorSpace", out OculusSplashColorSpace);

				switch (OculusSplashColorSpace)
				{
					case "Unmanaged":
						return "!Unmanaged";
					case "Rec_2020":
		...
		private void PackageForOculus(string UE4BuildPath, string ProjectDirectory)
		{
			...
			if (bPackageForOculus)
			{
				...
				{
					SafeDeleteFile(SplashTargetPath);
					MakeDirectoryIfRequired(SplashTargetPath);
		...
		private string GetOculusBuildSettings()
		{
			StringBuilder CurrentSettings = new StringBuilder();
			bool bPackageForOculus = IsPackagingForOculusMobile();
			...
			if (bPackageForOculus)
			{
				...
				string XrApi;
				Ini.GetString("/Script/OculusHMD.OculusHMDRuntimeSettings", "XrApi", out XrApi);
				CurrentSettings.AppendLine(string.Format("XrApi={0}", XrApi));
			}
			return CurrentSettings.ToString();
		}
		...
		private string GenerateManifest(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, string EngineDirectory, bool bIsForDistribution, bool bPackageDataInsideApk, string GameBuildFilesPath, bool bHasOBBFiles, bool bDisableVerifyOBBOnStartUp, string UE4Arch, string GPUArch, string CookFlavor, bool bUseExternalFilesDir, string Configuration, int SDKLevelInt, bool bIsEmbedded, bool bEnableBundle)
		{
			...
			if(bPackageForOculusMobile && OculusPackageHasOSSplash())
			{
				...
				string colorSpace = GetOculusSplashColorSpace();
				if(colorSpace != "")
				{
					Text.AppendLine("\t\t\t<meta-data android:name=\"com.oculus.ossplash.colorspace\" android:value=\"{0}\"/>", colorSpace);
				}
			}
			...
			UnrealTargetConfiguration Configuration, bool bMakeSeparateApks, bool bIncrementalPackage, bool bDisallowPackagingDataInApk, bool bDisallowExternalFilesDir, bool bSkipGradleBuild)
		{
			...
			string lldbVisualizerPath = Path.GetFullPath(Path.Combine(EngineDirectory, "Extras", "LLDBDataFormatters", "UE4DataFormatters_2ByteChars.py"));
			...
			Replacements.Add("${PY_VISUALIZER_PATH}", lldbVisualizerPath);
			...
			foreach (Tuple<string, string, string> build in BuildList)
			{
				...
				if (IsPackagingForOculusMobile() && Configuration != UnrealTargetConfiguration.Shipping)
				{
