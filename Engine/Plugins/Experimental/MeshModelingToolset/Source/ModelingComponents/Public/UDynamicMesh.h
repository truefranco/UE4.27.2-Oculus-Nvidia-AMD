...
//DECLARE_LOG_CATEGORY_EXTERN(LogDynamicMesh, Log, All);
class FDynamicMesh3;
/**
...
public:
	...
	MODELINGCOMPONENTS_API UPARAM(DisplayName = "Target") UDynamicMesh* Reset();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Dynamic Mesh")
	MODELINGCOMPONENTS_API UPARAM(DisplayName = "Target") UDynamicMesh* ResetToCube();
	 ...
	 */
	MODELINGCOMPONENTS_API virtual void InitializeMesh();

	/**
	 * @return true if the mesh has no triangles
	 */
	UFUNCTION(BlueprintCallable, Category = "Dynamic Mesh")
	MODELINGCOMPONENTS_API bool IsEmpty() const;

	/**
	 * @return number of triangles in the mesh
	 */
	UFUNCTION(BlueprintCallable, Category = "Dynamic Mesh")
	MODELINGCOMPONENTS_API UPARAM(DisplayName = "Triangle Count") int32 GetTriangleCount() const;
	 ...
	 */
	const FDynamicMesh3& GetMeshRef() const { return *DyMesh; }
	/**
	 * @return Const pointer to the internal FDynamicMesh3.
	 * @warning Calling ProcessMesh() is preferred! This interface may be  removed in the future
	 */
	const FDynamicMesh3* GetMeshPtr() const { return DyMesh.Get(); }
	 ...
	 */
	FDynamicMesh3& GetMeshRef() { return *DyMesh; }
	/**
	 * @return Writable pointer to the internal FDynamicMesh3.
	 * @warning Calling EditMesh() is preferred! This interface may be  removed in the future
	 */
	FDynamicMesh3* GetMeshPtr() { return DyMesh.Get(); }


	/** Replace the internal mesh with a copy of MoveMesh */
	MODELINGCOMPONENTS_API void SetMesh(const FDynamicMesh3& MoveMesh);

	/** Replace the internal mesh with the data in MoveMesh */
	MODELINGCOMPONENTS_API void SetMesh(FDynamicMesh3&& MoveMesh);

	/**
	 * Apply ProcessFunc to the internal Mesh
	 */
	MODELINGCOMPONENTS_API void ProcessMesh(TFunctionRef<void(const FDynamicMesh3&)> ProcessFunc) const;
	 ...
	 */
	MODELINGCOMPONENTS_API void EditMesh(TFunctionRef<void(FDynamicMesh3&)> EditFunc,
	 ...
	 */
	MODELINGCOMPONENTS_API TUniquePtr<FDynamicMesh3> ExtractMesh();
	...
	MODELINGCOMPONENTS_API void ApplyChange(const FMeshVertexChange* Change, bool bRevert);

	// IMeshCommandChangeTarget implementation, allows a FMeshChange to be applied to the mesh
	MODELINGCOMPONENTS_API void ApplyChange(const FMeshChange* Change, bool bRevert);

	// IMeshReplacementCommandChangeTarget implementation, allows a FMeshReplacementChange to be applied to the mesh
	MODELINGCOMPONENTS_API void ApplyChange(const FMeshReplacementChange* Change, bool bRevert);
...
public:
	 ...
	 */
	MODELINGCOMPONENTS_API virtual void PostRealtimeUpdate();
...
protected:
	 ...
	 */
	TUniquePtr<FDynamicMesh3> DyMesh;

	/**
	 * Allocate a new Mesh (ie pointer will change) and then call InitializeMesh()
	 */
	MODELINGCOMPONENTS_API void InitializeNewMesh();
	 ...
	 */
	MODELINGCOMPONENTS_API void EditMeshInternal(TFunctionRef<void(FDynamicMesh3&)> EditFunc, const FDynamicMeshChangeInfo& ChangeInfo, bool bDeferChangeEvents = false);
...
public:
	...
	MODELINGCOMPONENTS_API virtual void ExportCustomProperties(FOutputDevice& Out, uint32 Indent) override;
	MODELINGCOMPONENTS_API virtual void ImportCustomProperties(const TCHAR* SourceText, FFeedbackContext* Warn) override;
...
public:
	 ...
	 */
	MODELINGCOMPONENTS_API virtual void SetMeshGenerator(UDynamicMeshGenerator* NewGenerator);

	/**
	 * Set the active mesh generator. Clears if nullptr
	 */
	MODELINGCOMPONENTS_API virtual void ClearMeshGenerator();

	/**
	 * Reset() the mesh, which will re-run the active MeshGenerator, if bEnableMeshGenerator
	 */
	MODELINGCOMPONENTS_API virtual void Regenerate();
