...
static VkRenderPass CreateRenderPass(FVulkanDevice& InDevice, const FVulkanRenderTargetLayout& RTLayout)
{
	...
	ZeroVulkanStruct(CreateInfo, VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO_2_KHR);
	...
	VkSubpassDescription2 SubpassDescriptions[8];
	VkSubpassDependency2 SubpassDependencies[8];
	...
	// main sub-pass
	{
		...
		FMemory::Memzero(&SubpassDesc, sizeof(VkSubpassDescription2));

		SubpassDesc.sType = VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_2_KHR;
		SubpassDesc.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
		SubpassDesc.viewMask = RTLayout.GetIsMultiView() ? MultiviewMask : 0;
		...
		SubpassDesc.pResolveAttachments = (bDepthReadSubpass || bMobileTonemapSubpass) ? nullptr : RTLayout.GetResolveAttachmentReferences();
		SubpassDesc.pDepthStencilAttachment = RTLayout.GetDepthStencilAttachmentReference();
	}

	// Color write and depth read sub-pass
	VkAttachmentReference2 InputAttachments1[MaxSimultaneousRenderTargets + 1];
	if (bDepthReadSubpass)
	{
		...
		SubpassDesc.colorAttachmentCount = RTLayout.GetNumColorAttachments();
		SubpassDesc.pColorAttachments = RTLayout.GetColorAttachmentReferences();
		SubpassDesc.pResolveAttachments = bMobileTonemapSubpass ? nullptr : RTLayout.GetResolveAttachmentReferences();
		
		check(RTLayout.GetDepthStencilAttachmentReference());

		// depth as Input0
		InputAttachments1[0].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
		InputAttachments1[0].pNext = nullptr;
		...
		InputAttachments1[0].aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
		...
		FMemory::Memzero(&SubpassDep, sizeof(VkSubpassDependency2));

		SubpassDep.sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
	...
	VkAttachmentReference2 InputAttachments2[MaxSimultaneousRenderTargets + 1];
	...
	if (bDeferredShadingSubpass || (GIsEditor && bMobileTonemapSubpass))
	{
		...
		DepthStencilAttachment.sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
		...
		const VkAttachmentReference2* ColorRef = RTLayout.GetColorAttachmentReferences();
		...
		if (bDeferredShadingSubpass)
		{
			...
			InputAttachments2[0].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
			InputAttachments2[0].pNext = nullptr;
			InputAttachments2[0].attachment = DepthStencilAttachment.attachment;
			InputAttachments2[0].layout = DepthStencilAttachment.layout;
			InputAttachments2[0].aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
			InputAttachments2[1].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
			InputAttachments2[1].attachment = VK_ATTACHMENT_UNUSED;
			InputAttachments2[1].layout = VK_IMAGE_LAYOUT_UNDEFINED;
			for (int32 i = 2; i < 5; ++i)
			{
				InputAttachments2[i].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
				InputAttachments2[i].pNext = nullptr;
				...
				InputAttachments2[i].aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
			}
	...
	VkAttachmentReference2 InputAttachments3[MaxSimultaneousRenderTargets + 1];
	VkAttachmentReference2 ColorAttachments3[MaxSimultaneousRenderTargets + 1];
	if (bMobileTonemapSubpass)
	{
		...
		const VkAttachmentReference2* ColorRef = RTLayout.GetColorAttachmentReferences();
		const VkAttachmentReference2* ResolveRef = RTLayout.GetResolveAttachmentReferences();
		
		InputAttachments3[0].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
		InputAttachments3[0].pNext = nullptr;
		InputAttachments3[0].attachment = VK_ATTACHMENT_UNUSED;
		InputAttachments3[0].layout = VK_IMAGE_LAYOUT_UNDEFINED;
		InputAttachments3[0].aspectMask = 0;
		InputAttachments3[1].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
		InputAttachments3[1].pNext = nullptr;
		InputAttachments3[1].attachment = ColorRef[0].attachment;
		InputAttachments3[1].layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		InputAttachments3[1].aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		
		ColorAttachments3[0].sType = VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2_KHR;
		ColorAttachments3[0].pNext = nullptr;
		ColorAttachments3[0].attachment = ResolveRef[0].attachment;
		ColorAttachments3[0].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		ColorAttachments3[0].aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		
		SubpassDesc.sType = VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_2_KHR;
		SubpassDesc.flags = 0;
		SubpassDesc.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
		SubpassDesc.viewMask = RTLayout.GetIsMultiView() ? MultiviewMask : 0;
		SubpassDesc.inputAttachmentCount = 2;
		SubpassDesc.pInputAttachments = InputAttachments3;
		...
		if (InDevice.GetOptionalExtensions().HasQcomRenderPassShaderResolve)
		{
	...
	CreateInfo.correlatedViewMaskCount = RTLayout.GetIsMultiView() ? 1 : 0;
	CreateInfo.pCorrelatedViewMasks = RTLayout.GetIsMultiView() ? &MultiviewMask : nullptr;
...
void FVulkanDynamicRHI::VulkanSetQcomFragmentDensityMapOffsets(bool bUseOffsets, FIntPoint Offsets[2])
{
	if (bUseOffsets)
	{
