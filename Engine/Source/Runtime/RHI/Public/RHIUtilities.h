...
)
{
	...
	{
		ETextureCreateFlags AdditionalTargetableFlags = TexCreate_None;
		...
		if (RHIDiscardsMSAATextures(GMaxRHIShaderPlatform))
		{
			AdditionalTargetableFlags |= (TexCreate_Memoryless | TexCreate_Transient);
		}
		...
		OutTargetableTexture = RHICreateTexture2D(SizeX, SizeY, Format, NumMips, NumSamples, Flags | TargetableTextureFlags | AdditionalTargetableFlags, ERHIAccess::SRVMask, CreateInfo);
...
)
{
	...
	else
	{
		...
		// Create a texture that has TargetableTextureFlags set, and a second texture that has TexCreate_ResolveTargetable and TexCreate_ShaderResource set.
		OutTargetableTexture = RHICreateTexture2DArray(SizeX, SizeY, SizeZ, Format, NumMips, NumSamples, Flags | TargetableTextureFlags | AdditionalTargetableFlags, CreateInfo);
